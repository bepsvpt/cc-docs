...
| `includeGitInstructions`          | Inclure les instructions de workflow de commit et PR intégrées dans l'invite système de Claude (par défaut : `true`). Définir à `false` pour supprimer ces instructions, par exemple lors de l'utilisation de vos propres skills de workflow git. La variable d'environnement `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` a la priorité sur ce paramètre quand elle est définie                          | `false`                                                                 |
...
| `showTurnDuration`                | Afficher les messages de durée de tour après les réponses (par exemple, « Cooked for 1m 6s »). Définir à `false` pour masquer ces messages                                                                                                                                                                                                                                                          | `true`                                                                  |
...
| `terminalProgressBarEnabled`      | Activer la barre de progression du terminal qui affiche la progression dans les terminaux supportés comme Windows Terminal et iTerm2 (par défaut : `true`)                                                                                                                                                                                                                                          | `false`                                                                 |
...
| `disableBypassPermissionsMode` | Définir à `"disable"` pour empêcher l'activation du mode `bypassPermissions`. Ceci désactive l'indicateur de ligne de commande `--dangerously-skip-permissions`. Voir [paramètres gérés](/fr/permissions#managed-only-settings)                                                                      | `"disable"`                                                            |
...
Les chemins dans `filesystem.allowWrite`, `filesystem.denyWrite`, et `filesystem.denyRead` supportent ces préfixes :
...
La liste blanche supporte sept types de source de marketplace. La plupart des sources utilisent la correspondance exacte, tandis que `hostPattern` utilise la correspondance regex par rapport à l'hôte de la marketplace.
