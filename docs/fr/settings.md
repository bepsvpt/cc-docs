...
| `cleanupPeriodDays`               | Les sessions inactives pendant plus longtemps que cette période sont supprimées au démarrage (par défaut : 30 jours).<br /><br />Définir à `0` supprime tous les transcripts existants au démarrage et désactive complètement la persistance de session. Aucun nouveau fichier `.jsonl` n'est écrit, `/resume` n'affiche aucune conversation, et les hooks reçoivent un `transcript_path` vide.     | `20`                                                                    |
...
| `strictKnownMarketplaces`         | Quand défini dans managed-settings.json, liste blanche des marketplaces de plugins que les utilisateurs peuvent ajouter. Non défini = pas de restrictions, tableau vide = verrouillage. S'applique uniquement aux ajouts de marketplace. Voir [Restrictions de marketplace gérées](/fr/plugin-marketplaces#managed-marketplace-restrictions)                                                        | `[{ "source": "github", "repo": "acme-corp/plugins" }]`                 |
...
| `defaultMode`                  | [Mode de permission](/fr/permission-modes) par défaut lors de l'ouverture de Claude Code                                                                                                                                                                                                             | `"acceptEdits"`                                                        |
...
| `filesystem.allowManagedReadPathsOnly` | (Paramètres gérés uniquement) Seuls les chemins `allowRead` à partir des paramètres gérés sont respectés. Les entrées `allowRead` à partir des paramètres utilisateur, projet et locaux sont ignorées. Par défaut : false                                                                                                                                                                                                   | `true`                          |
