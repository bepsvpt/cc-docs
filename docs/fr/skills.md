...
<Note>
  Pour les commandes intégrées comme `/help` et `/compact`, voir [mode interactif](/fr/interactive-mode#built-in-commands).
...
Les skills groupées sont livrées avec Claude Code et sont disponibles dans chaque session. Contrairement aux [commandes intégrées](/fr/interactive-mode#built-in-commands), qui exécutent une logique fixe directement, les skills groupées sont basées sur des prompts : elles donnent à Claude un manuel détaillé et le laissent orchestrer le travail en utilisant ses outils. Cela signifie que les skills groupées peuvent générer des agents parallèles, lire des fichiers et s'adapter à votre base de code.

Vous invoquez les skills groupées de la même manière que n'importe quelle autre skill : tapez `/` suivi du nom de la skill.

* **`/simplify`** : examine vos fichiers récemment modifiés pour les problèmes de réutilisation de code, de qualité et d'efficacité, puis les corrige. Exécutez-la après avoir implémenté une fonctionnalité ou une correction de bug pour nettoyer votre travail. Elle génère trois agents d'examen en parallèle (réutilisation de code, qualité du code, efficacité), agrège leurs conclusions et applique les corrections. Passez du texte optionnel pour vous concentrer sur des préoccupations spécifiques : `/simplify focus on memory efficiency`.

* **`/batch <instruction>`** : orchestre les changements à grande échelle dans une base de code en parallèle. Fournissez une description du changement et `/batch` recherche la base de code, décompose le travail en 5 à 30 unités indépendantes, et présente un plan pour votre approbation. Une fois approuvé, il génère un agent d'arrière-plan par unité, chacun dans un [git worktree](/fr/common-workflows#run-parallel-claude-code-sessions-with-git-worktrees) isolé. Chaque agent implémente son unité, exécute les tests et ouvre une demande de tirage. Nécessite un référentiel git. Exemple : `/batch migrate src/ from Solid to React`.

* **`/debug [description]`** : dépanne votre session Claude Code actuelle en lisant le journal de débogage de la session. Décrivez optionnellement le problème pour concentrer l'analyse.

* **`/loop [interval] <prompt>`** : exécute un prompt à plusieurs reprises à un intervalle donné tant que la session reste ouverte. Claude analyse l'intervalle, planifie une tâche cron récurrente et confirme la cadence. Utile pour interroger un déploiement, surveiller une demande de tirage ou réexécuter périodiquement une autre skill. Exemple : `/loop 5m check if the deploy finished`. Voir [Exécuter des prompts selon un calendrier](/fr/scheduled-tasks).
...
<Steps>
  <Step title="Créez le répertoire de la skill">
  ...
  <Step title="Testez la skill">
    Vous pouvez la tester de deux façons :

    **Laissez Claude l'invoquer automatiquement** en demandant quelque chose qui correspond à la description :

    ```text  theme={null}
    How does this code work?
    ```

    **Ou invoquez-la directement** avec le nom de la skill :
...
Le `SKILL.md` contient les instructions principales et est obligatoire. Les autres fichiers sont optionnels et vous permettent de créer des skills plus puissantes : des modèles pour que Claude remplisse, des exemples de sortie montrant le format attendu, des scripts que Claude peut exécuter, ou une documentation de référence détaillée. Référencez ces fichiers à partir de votre `SKILL.md` pour que Claude sache ce que chaque fichier contient et quand le charger. Voir [Ajouter des fichiers de support](#add-supporting-files) pour plus de détails.
...
Les skills définies dans `.claude/skills/` dans les répertoires ajoutés via `--add-dir` sont chargées automatiquement et détectées par la détection de changement en direct, vous pouvez donc les modifier pendant une session sans redémarrer.
...
| `disable-model-invocation` | Non         | Définissez sur `true` pour empêcher Claude de charger automatiquement cette skill. Utilisez pour les workflows que vous voulez déclencher manuellement avec `/name`. Par défaut : `false`. |
| `user-invocable`           | Non         | Définissez sur `false` pour masquer du menu `/`. Utilisez pour les connaissances d'arrière-plan que les utilisateurs ne devraient pas invoquer directement. Par défaut : `true`.           |
...
| `context`                  | Non         | Définissez sur `fork` pour exécuter dans un contexte de subagent forké.                                                                                                                    |
...
### Contrôlez qui invoque une skill

Par défaut, vous et Claude pouvez invoquer n'importe quelle skill. Vous pouvez taper `/skill-name` pour l'invoquer directement, et Claude peut la charger automatiquement quand c'est pertinent pour votre conversation. Deux champs du frontmatter vous permettent de restreindre ceci :
...
* **`user-invocable: false`** : Seul Claude peut invoquer la skill. Utilisez ceci pour les connaissances d'arrière-plan qui ne sont pas actionnables en tant que commande. Une skill `legacy-system-context` explique comment fonctionne un ancien système. Claude devrait le savoir quand c'est pertinent, mais `/legacy-system-context` n'est pas une action significative pour les utilisateurs.
...
<Note>
  Dans une session régulière, les descriptions de skills sont chargées dans le contexte pour que Claude sache ce qui est disponible, mais le contenu complet de la skill ne se charge que quand invoquée. [Les subagents avec des skills préchargées](/fr/sub-agents#preload-skills-into-subagents) fonctionnent différemment : le contenu complet de la skill est injecté au démarrage.
</Note>
...
### Passez des arguments aux skills

Vous et Claude pouvez passer des arguments lors de l'invocation d'une skill. Les arguments sont disponibles via l'espace réservé `$ARGUMENTS`.
...
### Injectez du contexte dynamique
...
Cette skill résume une demande de tirage en récupérant les données de demande de tirage en direct avec le CLI GitHub. Les commandes `!`gh pr diff\`\` et autres s'exécutent d'abord, et leur sortie est insérée dans le prompt :
...
3. Claude reçoit le prompt complètement rendu avec les données réelles de la demande de tirage
...
### Exécutez les skills dans un subagent
...
<Warning>
  `context: fork` n'a de sens que pour les skills avec des instructions explicites. Si votre skill contient des directives comme ' utilisez ces conventions d'API ' sans une tâche, le subagent reçoit les directives mais aucun prompt actionnable, et retourne sans sortie significative.
</Warning>
...
| :--------------------------- | :---------------------------------------- | :------------------------------ | :----------------------------- |
| Skill avec `context: fork`   | Du type d'agent (`Explore`, `Plan`, etc.) | Contenu SKILL.md                | CLAUDE.md                      |
...
**Désactivez toutes les skills** en refusant l'outil Skill dans `/permissions` :
...
**Autorisez ou refusez des skills spécifiques** en utilisant les [règles de permission](/fr/permissions) :
...
**Masquez les skills individuelles** en ajoutant `disable-model-invocation: true` à leur frontmatter. Cela supprime la skill du contexte de Claude entièrement.
...
Créez `~/.claude/skills/codebase-visualizer/SKILL.md`. La description indique à Claude quand activer cette Skill, et les instructions indiquent à Claude d'exécuter le script fourni :
...
* Une **arborescence réductible** où vous pouvez développer et réduire les répertoires, avec des indicateurs de type de fichier codés par couleur
...
Ce modèle fonctionne pour n'importe quelle sortie visuelle : graphiques de dépendances, rapports de couverture de test, documentation d'API ou visualisations de schéma de base de données. Le script fourni fait le gros du travail tandis que Claude gère l'orchestration.
...
* **[Subagents](/fr/sub-agents)** : déléguez les tâches à des agents spécialisés
* **[Plugins](/fr/plugins)** : empaquetez et distribuez les skills avec d'autres extensions
* **[Hooks](/fr/hooks)** : automatisez les workflows autour des événements d'outils
* **[Memory](/fr/memory)** : gérez les fichiers CLAUDE.md pour le contexte persistant
* **[Mode interactif](/fr/interactive-mode#built-in-commands)** : commandes intégrées et raccourcis
* **[Permissions](/fr/permissions)** : contrôlez l'accès aux outils et aux skills
