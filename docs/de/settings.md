...
| `cleanupPeriodDays`               | Sitzungen, die länger als dieser Zeitraum inaktiv sind, werden beim Start gelöscht (Standard: 30 Tage, Minimum 1). Das Setzen auf `0` wird mit einem Validierungsfehler abgelehnt. Um Transkriptschreibvorgänge vollständig im nicht-interaktiven Modus (`-p`) zu deaktivieren, verwenden Sie das Flag `--no-session-persistence` oder die SDK-Option `persistSession: false`; es gibt kein interaktives Äquivalent.                                                                                                                                                                                                                                        | `20`                                                                                                                             |
...
| `editorMode`                 | Tastaturkürzel-Modus für die Eingabeaufforderung: `"normal"` oder `"vim"`. Standard: `"normal"`. Wird automatisch geschrieben, wenn Sie `/vim` ausführen. Wird in `/config` als **Key binding mode** angezeigt                                                                                                                                                                   | `"vim"`        |
...
| `disableBypassPermissionsMode`      | Setzen Sie auf `"disable"`, um zu verhindern, dass der `bypassPermissions`-Modus aktiviert wird. Dies deaktiviert das Befehlszeilenflag `--dangerously-skip-permissions`. Am nützlichsten in [verwalteten Einstellungen](/de/permissions#managed-settings), wo Benutzer es nicht überschreiben können                                                                            | `"disable"`                                                            |
