...
| `vim.mode`                                                                       | 啟用 [vim 模式](/zh-TW/interactive-mode#vim-editor-mode)時的目前 vim 模式（`NORMAL`、`INSERT`、`VISUAL` 或 `VISUAL LINE`）                             |
...
如需了解快取欄位的含義及其計費方式，請參閱[檢查快取效能](/zh-TW/prompt-caching#check-cache-performance)。
