...
| **限制令牌預算**           | 設定 [`MAX_THINKING_TOKENS`](/zh-TW/env-vars) 環境變數                              | 將思考預算限制為特定數量的令牌（在 Opus 4.6 和 Sonnet 4.6 上被忽略，除非設定為 0）。範例：`export MAX_THINKING_TOKENS=10000` |
...
**使用較舊的模型**，思考使用固定預算，最多 31,999 個令牌來自您的輸出預算。您可以使用 [`MAX_THINKING_TOKENS`](/zh-TW/env-vars) 環境變數限制此，或通過 `/config` 或 `Option+T`/`Alt+T` 切換完全禁用思考。

`MAX_THINKING_TOKENS` 在 Opus 4.6 和 Sonnet 4.6 上被忽略，因為自適應推理控制思考深度。唯一的例外：設定 `MAX_THINKING_TOKENS=0` 仍會在任何模型上完全禁用思考。要禁用自適應思考並恢復為固定思考預算，請設定 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`。請參閱[環境變數](/zh-TW/env-vars)。
...
Worktree 隔離預設使用 git。對於其他版本控制系統（如 SVN、Perforce 或 Mercurial），配置 [WorktreeCreate 和 WorktreeRemove hooks](/zh-TW/hooks#worktreecreate) 以提供自訂 worktree 建立和清理邏輯。配置後，當您使用 `--worktree` 時，這些 hooks 會取代預設 git 行為。
