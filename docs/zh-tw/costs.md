...
延伸思考預設為啟用，預算為 31,999 個 token，因為它可以顯著改善複雜規劃和推理任務的效能。但是，思考 token 會作為輸出 token 計費，因此對於不需要深度推理的較簡單任務，您可以透過在 `/model` 中降低 Opus 4.6 的[努力等級](/zh-TW/model-config#adjust-effort-level)、在 `/config` 中停用思考或降低預算（例如，`MAX_THINKING_TOKENS=8000`）來降低成本。
