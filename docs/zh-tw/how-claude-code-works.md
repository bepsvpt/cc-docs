...
本指南涵蓋核心架構、內建功能，以及[有效使用 Claude Code 的提示](#work-effectively-with-claude-code)。如需逐步說明，請參閱[常見工作流程](/zh-TW/common-workflows)。如需 skills、MCP 和 hooks 等擴充功能，請參閱[擴充 Claude Code](/zh-TW/features-overview)。
...
代理迴圈由兩個元件提供支援：[推理的模型](#models)和[執行的工具](#tools)。Claude Code 充當 Claude 周圍的**代理工具**：它提供工具、上下文管理和執行環境，將語言模型轉變為能力強大的編碼代理。
...
| **網路**    | 搜尋網路、擷取文件、查詢錯誤訊息                                                                  |
| **程式碼智慧** | 編輯後查看類型錯誤和警告、跳轉到定義、尋找參考（需要[程式碼智慧外掛程式](/zh-TW/discover-plugins#code-intelligence)） |

這些是主要功能。Claude 還具有用於生成 subagents、詢問您問題和其他協調任務的工具。請參閱[Claude 可用的工具](/zh-TW/settings#tools-available-to-claude)以取得完整清單。
...
**擴充基本功能：** 內建工具是基礎。您可以使用 [skills](/zh-TW/skills) 擴充 Claude 知道的內容、使用 [MCP](/zh-TW/mcp) 連接到外部服務、使用 [hooks](/zh-TW/hooks) 自動化工作流程，以及使用 [subagents](/zh-TW/sub-agents) 卸載任務。這些擴充形成了核心代理迴圈之上的一層。請參閱[擴充 Claude Code](/zh-TW/features-overview) 以獲得有關為您的需求選擇正確擴充的指導。

## Claude 可以存取什麼

本指南著重於終端機。Claude Code 也在 [VS Code](/zh-TW/vs-code)、[JetBrains IDE](/zh-TW/jetbrains) 和其他環境中執行。
...
* **您設定的擴充。** 用於外部服務的 [MCP servers](/zh-TW/mcp)、用於工作流程的 [skills](/zh-TW/skills)、用於委派工作的 [subagents](/zh-TW/sub-agents)，以及用於瀏覽器互動的 [Claude in Chrome](/zh-TW/chrome)。
...
您可以透過終端機、[桌面應用程式](/zh-TW/desktop)、[IDE 擴充](/zh-TW/ide-integrations)、[claude.ai/code](https://claude.ai/code)、[遠端控制](/zh-TW/remote-control)、[Slack](/zh-TW/slack) 和 [CI/CD 管道](/zh-TW/github-actions)存取 Claude Code。介面決定了您如何看到和與 Claude 互動，但底層的代理迴圈是相同的。請參閱[在任何地方使用 Claude Code](/zh-TW/overview#use-claude-code-everywhere) 以取得完整清單。

## 使用會話

Claude Code 在您工作時將您的對話儲存在本機。每條訊息、工具使用和結果都被儲存，這使得[倒帶](#undo-changes-with-checkpoints)、[恢復和分叉](#resume-or-fork-sessions)會話成為可能。在 Claude 進行程式碼變更之前，它還會快照受影響的檔案，以便您在需要時可以還原。
...
由於會話與目錄相關，您可以使用 [git worktrees](/zh-TW/common-workflows#run-parallel-claude-code-sessions-with-git-worktrees) 執行平行 Claude 會話，這會為個別分支建立單獨的目錄。

### 恢復或分叉會話

當您使用 `claude --continue` 或 `claude --resume` 恢復會話時，您使用相同的會話 ID 從中斷的地方繼續。新訊息附加到現有對話。您的完整對話歷史被還原，但會話範圍的許可不被還原。您需要重新批准這些。

<img src="https://mintcdn.com/claude-code/c5r9_6tjPMzFdDDT/images/session-continuity.svg?fit=max&auto=format&n=c5r9_6tjPMzFdDDT&q=85&s=fa41d12bfb57579cabfeece907151d30" alt="會話連續性：恢復繼續相同的會話，分叉建立具有新 ID 的新分支。" width="560" height="280" data-path="images/session-continuity.svg" />
...
Claude 的上下文視窗保存您的對話歷史、檔案內容、命令輸出、[CLAUDE.md](/zh-TW/memory)、載入的 skills 和系統指示。當您工作時，上下文會填滿。Claude 會自動壓縮，但對話早期的指示可能會丟失。將持久規則放在 CLAUDE.md 中，並執行 `/context` 以查看什麼在佔用空間。
...
<CardGroup cols={2}>
  <Card title="使用功能進行擴充" icon="puzzle-piece" href="/zh-TW/features-overview">
