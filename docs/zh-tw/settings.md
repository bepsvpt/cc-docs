...
| `defaultMode`                  | 開啟 Claude Code 時的預設[權限模式](/zh-TW/permission-modes)                                                                                                    | `"acceptEdits"`                                                        |
...
| `filesystem.allowManagedReadPathsOnly` | （Managed 設定僅限）僅尊重 managed 設定中的 `allowRead` 路徑。來自使用者、專案和本機設定的 `allowRead` 項目會被忽略。預設：false                                                                                             | `true`                          |
