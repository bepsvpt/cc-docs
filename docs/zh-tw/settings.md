...
| `minimumVersion`                  | 防止自動更新程式降級到特定版本以下。當切換到穩定頻道並選擇保持在目前版本直到穩定趕上時自動設定。與 `autoUpdatesChannel` 一起使用                                                                                                                                                                                                                            | `"2.1.85"`                                                                                                                    |
...
| `viewMode`                        | 啟動時的預設文字記錄檢視模式：`"default"`、`"verbose"` 或 `"focus"`。設定時覆蓋粘性 Ctrl+O 選擇                                                                                                                                                                                                                                   | `"verbose"`                                                                                                                   |
...
| `editorMode`                 | 輸入提示的快捷鍵模式：`"normal"` 或 `"vim"`。預設：`"normal"`。在 `/config` 中顯示為**快捷鍵模式**                                                                                                               | `"vim"`        |
