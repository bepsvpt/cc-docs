...
* `${CLAUDE_PLUGIN_ROOT}`：外掛程式的根目錄，用於與 [plugin](/zh-TW/plugins) 一起打包的指令碼。
...
| WorktreeRemove、Notification、SessionEnd、PreCompact、PostCompact、InstructionsLoaded | 無                       | 無決定控制。用於副作用，如記錄或清理                                                                                 |
...
當主 Claude Code 代理完成回應時執行。如果停止是由於使用者中斷，則不執行。
...
SessionEnd hooks 的預設逾時為 1.5 秒。這適用於工作階段退出和 `/clear`。如果您的 hooks 需要更多時間，請將 `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` 環境變數設定為毫秒的更高值。任何每個 hook 的 `timeout` 設定也受此值的限制。
