...
本逐步指南將引導您使用 `/agent` 命令建立使用者層級的 subagent。該 subagent 審查程式碼並為程式碼庫提出改進建議。
...
<Steps>
  ...
  <Step title="使用 Claude 生成">
    ...
    Claude 生成系統提示和配置。按 `e` 在編輯器中開啟它，如果您想自訂它。
  </Step>
...
| :-------------------- | :-------- | :------- | :------------------------------ |
| `--agents` CLI 標誌     | 目前工作階段    | 1（最高）    | 啟動 Claude Code 時傳遞 JSON         |
| `.claude/agents/`     | 目前專案      | 2        | 互動式或手動                          |
| `~/.claude/agents/`   | 所有您的專案    | 3        | 互動式或手動                          |
| Plugin 的 `agents/` 目錄 | 啟用外掛程式的位置 | 4（最低）    | 使用 [plugins](/zh-TW/plugins) 安裝 |
...
`--agents` 標誌接受 JSON，具有與基於檔案的 subagents 相同的 [frontmatter](#supported-frontmatter-fields) 欄位：`description`、`prompt`、`tools`、`disallowedTools`、`model`、`permissionMode`、`mcpServers`、`hooks`、`maxTurns`、`skills` 和 `memory`。使用 `prompt` 作為系統提示，等同於基於檔案的 subagents 中的 markdown 主體。
...
| `mcpServers`      | No       | [MCP servers](/zh-TW/mcp) 可用於此 subagent。每個條目要麼是參考已配置伺服器的伺服器名稱（例如，`"slack"`），要麼是內聯定義，其中伺服器名稱為鍵，完整 [MCP server config](/zh-TW/mcp#configure-mcp-servers) 為值                     |
...
`permissionMode` 欄位控制 subagent 如何處理權限提示。Subagents 從主要對話繼承權限上下文，但可以覆蓋模式。
...
如果父級使用 `bypassPermissions`，這優先並且無法被覆蓋。
...
* Subagent 的系統提示還包括記憶目錄中 `MEMORY.md` 的前 200 行，以及如果超過 200 行則策劃 `MEMORY.md` 的說明。
...
您可以透過將 subagents 新增到 [settings](/zh-TW/settings#permission-settings) 中的 `permissions.deny` 陣列來防止 Claude 使用特定 subagents。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 與 subagent 的 name 欄位相符。
...
如果背景 subagent 因權限遺失而失敗，您可以 [恢復它](#resume-subagents) 在前景中以使用互動式提示重試。
