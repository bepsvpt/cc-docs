...
由啟用的 [plugin](/zh-TW/plugins) 提供的 Subagents 在預輸入中顯示為 `<plugin-name>:<agent-name>`。您也可以手動輸入提及而不使用選擇器：`@agent-<name>` 用於本地 subagents，或 `@agent-<plugin-name>:<agent-name>` 用於外掛程式 subagents。
...
當 subagent 完成時，Claude 接收其代理 ID。Claude 使用 `SendMessage` 工具，將代理的 ID 作為 `to` 欄位來恢復它。若要恢復 subagent，請要求 Claude 繼續先前的工作：
