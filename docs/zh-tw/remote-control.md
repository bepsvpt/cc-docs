...
<Tabs>
  ...
  <Tab title="VS Code">
    在 [Claude Code VS Code 擴充功能](/zh-TW/vs-code)中，在提示框中輸入 `/remote-control` 或 `/rc`，或使用 `/` 開啟命令選單並選擇它。需要 Claude Code v2.1.79 或更新版本。
...
如果您沒有設定明確名稱，標題會在您發送提示後更新以反映您的提示。
...
<Note>
  行動推播通知需要 Claude Code v2.1.110 或更新版本。
</Note>
...
<Steps>
  ...
  <Step title="在 Claude Code 中啟用推播">
    在您的終端機中，執行 `/config` 並啟用**當 Claude 決定時推播**。
  </Step>
</Steps>
...
* [Ultraplan](/zh-TW/ultraplan)：從您的終端機啟動雲端規劃會話，並在瀏覽器中檢查計畫
* [Channels](/zh-TW/channels)：將 Telegram、Discord 或 iMessage 轉發到會話中，以便 Claude 在您離開時對訊息做出反應
