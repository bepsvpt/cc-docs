...
| `cleanupPeriodDays`               | Sesi yang tidak aktif lebih lama dari periode ini dihapus saat startup (default: 30 hari).<br /><br />Pengaturan ke `0` menghapus semua transkrip yang ada saat startup dan menonaktifkan persistensi sesi sepenuhnya. File `.jsonl` baru tidak ditulis, `/resume` tidak menunjukkan percakapan, dan hooks menerima `transcript_path` kosong.        | `20`                                                                    |
...
| `terminalProgressBarEnabled`      | Aktifkan bilah kemajuan terminal yang menunjukkan kemajuan di terminal yang didukung seperti Windows Terminal dan iTerm2 (default: `true`)                                                                                                                                                                                                           | `false`                                                                 |
...
| `teammateMode`                    | Bagaimana [rekan tim agent](/id/agent-teams) ditampilkan: `auto` (memilih panel terpisah di tmux atau iTerm2, dalam proses sebaliknya), `in-process`, atau `tmux`. Lihat [atur agent teams](/id/agent-teams#set-up-agent-teams)                                                                                                                      | `"in-process"`                                                          |
...
| `disableBypassPermissionsMode` | Atur ke `"disable"` untuk mencegah mode `bypassPermissions` diaktifkan. Ini menonaktifkan flag baris perintah `--dangerously-skip-permissions`. Lihat [pengaturan yang dikelola](/id/permissions#managed-only-settings)                             | `"disable"`                                                            |
...
1. **Pengaturan yang dikelola** ([yang dikelola server](/id/server-managed-settings), [kebijakan tingkat MDM/OS](#configuration-scopes), atau [pengaturan yang dikelola](/id/settings#settings-files))
   ...
   * Dalam tingkat yang dikelola, prioritas adalah: yang dikelola server > kebijakan tingkat MDM/OS > `managed-settings.json` > registry HKCU (Windows saja). Hanya satu sumber yang dikelola yang digunakan; sumber tidak digabungkan.
