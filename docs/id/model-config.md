...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | Model yang digunakan untuk semua [subagents](/id/sub-agents#choose-a-model) dan [agent teams](/id/agent-teams). Mengganti baik parameter `model` per-invocation maupun frontmatter `model` definisi subagent. Atur ke `inherit` untuk menggunakan resolusi model normal sebagai gantinya |
