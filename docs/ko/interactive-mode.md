...
| `Ctrl+O`                                          | 상세 출력 토글                            | 자세한 도구 사용 및 실행 표시. MCP 읽기 및 검색 호출도 확장하며, 기본적으로 "Slack 쿼리됨"과 같은 단일 줄로 축소됩니다                                                    |
...
| `Option+T` (macOS) 또는 `Alt+T` (Windows/Linux)     | 확장 사고 토글                            | 확장 사고 모드 활성화 또는 비활성화. 이 단축키를 활성화하려면 먼저 `/terminal-setup`을 실행하세요                                                               |
...
| :------------------- | :------------------------------------------------------------------------------------------------------- |
| `Ctrl+E`             | 모든 콘텐츠 표시 토글                                                                                             |
| `q`, `Ctrl+C`, `Esc` | 트랜스크립트 보기 종료. `Ctrl+C` 및 `Esc`는 [`transcript:exit`](/ko/keybindings)를 통해 재바인딩할 수 있습니다; `q`는 재바인딩할 수 없습니다 |
