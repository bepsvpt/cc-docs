...
<Steps>
  ...
  <Step title="저장 및 시도">
    Subagent를 저장합니다. 즉시 사용 가능합니다(재시작 필요 없음). 시도해 봅니다:
...
`--agents` 플래그는 파일 기반 subagent와 동일한 [frontmatter](#supported-frontmatter-fields) 필드를 가진 JSON을 허용합니다: `description`, `prompt`, `tools`, `disallowedTools`, `model`, `permissionMode`, `mcpServers`, `hooks`, `maxTurns`, `skills`, `memory`. 시스템 프롬프트에는 `prompt`를 사용하며, 이는 파일 기반 subagent의 markdown 본문과 동등합니다.
...
`permissionMode` 필드는 subagent가 권한 프롬프트를 처리하는 방식을 제어합니다. Subagent는 주 대화의 권한 컨텍스트를 상속하지만 모드를 재정의할 수 있습니다.
...
<Warning>
  `bypassPermissions`는 주의해서 사용하세요. 모든 권한 확인을 건너뛰어 subagent가 승인 없이 모든 작업을 실행할 수 있습니다.
</Warning>
...
Background subagent가 권한 부족으로 인해 실패하면 [resume it](#resume-subagents)을 foreground에서 수행하여 대화형 프롬프트로 다시 시도할 수 있습니다.
