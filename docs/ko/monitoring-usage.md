...
| `OTEL_METRICS_INCLUDE_ACCOUNT_UUID` | 메트릭에 user.account\_uuid 속성 포함 | `true`  | `false` |
...
* MCP 서버/도구 이름 및 스킬 이름은 사용자별 구성을 드러낼 수 있기 때문에 기본적으로 기록되지 않습니다. 이를 포함하려면 `OTEL_LOG_TOOL_DETAILS=1`을 설정합니다
