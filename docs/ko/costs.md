...
확장 사고는 기본적으로 31,999 토큰의 예산으로 활성화되어 있습니다. 복잡한 계획 및 추론 작업의 성능을 크게 향상시키기 때문입니다. 그러나 사고 토큰은 출력 토큰으로 청구되므로, 깊은 추론이 필요하지 않은 더 간단한 작업의 경우, `/model`에서 Opus 4.6의 [노력 수준](/ko/model-config#adjust-effort-level)을 낮추거나, `/config`에서 사고를 비활성화하거나, 예산을 낮춤으로써(예: `MAX_THINKING_TOKENS=8000`) 비용을 줄일 수 있습니다.
