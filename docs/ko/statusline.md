...
상태 표시줄은 Claude Code 하단의 사용자 정의 가능한 표시줄로, 구성한 모든 셸 스크립트를 실행합니다. stdin을 통해 JSON 세션 데이터를 수신하고 스크립트가 출력하는 모든 내용을 표시하여 컨텍스트 사용량, 비용, git 상태 또는 추적하려는 다른 항목을 한눈에 볼 수 있는 지속적인 보기를 제공합니다.
...
다음은 첫 번째 줄에 git 정보를 표시하고 두 번째 줄에 색상으로 구분된 컨텍스트 표시줄을 표시하는 [다중 줄 상태 표시줄](#display-multiple-lines)의 예입니다.
...
[`/statusline` 명령](#use-the-statusline-command)을 사용하여 Claude Code가 스크립트를 생성하도록 하거나, [수동으로 스크립트를 생성](#manually-configure-a-status-line)하여 설정에 추가합니다.
...
사용자 설정(`~/.claude/settings.json`, 여기서 `~`는 홈 디렉토리) 또는 [프로젝트 설정](/ko/settings#settings-files)에 `statusLine` 필드를 추가합니다. `type`을 `"command"`로 설정하고 `command`를 스크립트 경로 또는 인라인 셸 명령으로 지정합니다. 스크립트 생성에 대한 전체 설명은 [단계별로 상태 표시줄 빌드](#build-a-status-line-step-by-step)를 참조하세요.
...
* **색상**: `\033[32m`(녹색)과 같은 [ANSI 이스케이프 코드](https://en.wikipedia.org/wiki/ANSI_escape_code#Colors)를 사용합니다(터미널이 지원해야 함). [git 상태 예제](#git-status-with-colors)를 참조하세요.
...
| `context_window.context_window_size`                                      | 최대 컨텍스트 윈도우 크기(토큰). 기본값은 200000이거나 확장된 컨텍스트가 있는 모델의 경우 1000000입니다.                                  |
