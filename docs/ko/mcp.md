...
Claude Code는 MCP 도구 설명이 컨텍스트 윈도우의 10% 이상을 차지할 때 자동으로 Tool Search를 활성화합니다. [이 임계값을 조정](#configure-tool-search)하거나 Tool Search를 완전히 비활성화할 수 있습니다. 트리거될 때:
...
| 값          | 동작                                                  |
| :--------- | :-------------------------------------------------- |
| (설정되지 않음)  | 기본적으로 활성화됨. `ANTHROPIC_BASE_URL`이 비 자사 호스트일 때 비활성화됨 |
| `true`     | 항상 활성화, 비 자사 `ANTHROPIC_BASE_URL` 포함                |
| `auto`     | MCP 도구가 컨텍스트의 10%를 초과할 때 활성화                        |
