...
세 가지 수준이 세션 전체에 유지됩니다: **low**, **medium**, **high**. 네 번째 수준인 **max**는 토큰 지출에 제약이 없어 가장 깊은 추론을 제공하므로 응답이 더 느리고 `high`보다 비용이 더 많이 듭니다. `max`는 Opus 4.6에서만 사용 가능하며 세션 전체에 유지되지 않으며 `CLAUDE_CODE_EFFORT_LEVEL` 환경 변수를 통해서만 유지됩니다.
