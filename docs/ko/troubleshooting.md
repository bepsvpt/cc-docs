...
여러 Claude Code 설치는 버전 불일치 또는 예기치 않은 동작을 유발할 수 있습니다. 설치된 항목을 확인하세요:
...
이는 설치 디렉토리가 셸의 검색 경로에 없음을 의미합니다. 각 플랫폼에서 수정하려면 [PATH 확인](#verify-your-path)을 참조하세요.
...
2. **다른 프로세스 종료** 설치 전에 메모리를 확보하세요.

3. **더 큰 인스턴스 사용** 가능한 경우. Claude Code는 최소 4GB의 RAM이 필요합니다.
...
Windows의 네이티브 Claude Code는 Git Bash를 포함하는 [Git for Windows](https://git-scm.com/downloads/win)가 필요합니다.
...
특정 명령을 반복적으로 승인해야 하는 경우 `/permissions` 명령을 사용하여 특정 도구가 승인 없이 실행되도록 허용할 수 있습니다. [권한 문서](/ko/permissions#manage-permissions)를 참조하세요.
...
그런 다음 [환경](/ko/settings#environment-variables)에서 `USE_BUILTIN_RIPGREP=0`을 설정하세요.
...
2. **사후 처리 hooks 사용**: 누락된 언어 태그를 감지하고 추가하는 자동 형식 지정 hooks를 설정하세요. [편집 후 자동 형식 지정](/ko/hooks-guide#auto-format-code-after-edits)에서 PostToolUse 형식 지정 hook의 예를 참조하세요.
...
* **프로젝트 규칙 사용**: 선호하는 markdown 스타일을 [`CLAUDE.md`](/ko/memory)에 문서화하세요
