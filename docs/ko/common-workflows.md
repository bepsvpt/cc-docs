...
| **토큰 예산 제한**         | [`MAX_THINKING_TOKENS`](/ko/env-vars) 환경 변수 설정                                | 사고 예산을 특정 토큰 수로 제한 (Opus 4.6 및 Sonnet 4.6에서 0으로 설정하지 않으면 무시됨). 예: `export MAX_THINKING_TOKENS=10000` |
...
**다른 모델의 경우**, 사고는 출력 예산에서 최대 31,999개 토큰의 고정 예산을 사용합니다. [`MAX_THINKING_TOKENS`](/ko/env-vars) 환경 변수로 이를 제한하거나 `/config` 또는 `Option+T`/`Alt+T` 토글을 통해 사고를 완전히 비활성화할 수 있습니다.

`MAX_THINKING_TOKENS`는 Opus 4.6 및 Sonnet 4.6에서 무시되며, 적응형 추론이 사고 깊이를 제어하기 때문입니다. 한 가지 예외: `MAX_THINKING_TOKENS=0`을 설정하면 모든 모델에서 사고를 완전히 비활성화합니다. 적응형 사고를 비활성화하고 고정 사고 예산으로 되돌리려면 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`을 설정하십시오. [환경 변수](/ko/env-vars)를 참조하십시오.

<Warning>
  Claude 4 모델이 요약된 사고를 표시하더라도 사용된 모든 사고 토큰에 대해 요금이 청구됩니다
</Warning>
