...
[권한 구성](/ko/permissions) 및 [샌드박싱 활성화](/ko/sandboxing)에 대해 자세히 읽어보십시오.
...
Claude가 hooks를 작성할 수 있습니다. *"모든 파일 편집 후 eslint를 실행하는 hook 작성"* 또는 \*"마이그레이션 폴더에 대한 쓰기를 차단하는 hook 작성"\*과 같은 프롬프트를 시도해보십시오. `/hooks`를 실행하여 대화형 구성을 하거나 `.claude/settings.json`을 직접 편집하십시오.
