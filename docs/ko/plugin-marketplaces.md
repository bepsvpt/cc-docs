...
| `Invalid JSON syntax: Unexpected token...`        | JSON 구문 오류            | 누락된 쉼표, 추가 쉼표 또는 인용되지 않은 문자열 확인                  |
| `Duplicate plugin name "x" found in marketplace`  | 두 플러그인이 동일한 이름을 공유합니다 | 각 플러그인에 고유한 `name` 값 지정                          |
| `plugins[0].source: Path traversal not allowed`   | 소스 경로에 `..` 포함        | 마켓플레이스 루트에 상대적인 경로를 `..` 없이 사용                   |
