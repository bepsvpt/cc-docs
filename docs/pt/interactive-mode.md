...
<Note>
  Os atalhos de teclado podem variar por plataforma e terminal. Em [renderização em tela cheia](/pt/fullscreen), pressione `?` no visualizador de transcrição para ver os atalhos disponíveis lá.
...
| `Ctrl+X Ctrl+K`                                   | Encerrar todos os [subagentes em segundo plano](/pt/sub-agents#run-subagents-in-foreground-or-background) nesta sessão. Pressione duas vezes em 3 segundos para confirmar | Controle de subagente                                                                                                                                                                                                                                                                                                                       |
...
| `Esc`                                             | Interromper Claude                                                                                                                                                        | Pare a resposta atual ou chamada de ferramenta no meio da vez para que você possa redirecionar. Claude mantém o trabalho realizado até agora                                                                                                                                                                                                |
...
| `Option+T` (macOS) ou `Alt+T` (Windows/Linux)     | Alternar pensamento estendido                                                                                                                                             | Ativar ou desativar modo de pensamento estendido. {/* min-version: 2.1.132 */}A partir da v2.1.132, este atalho funciona no macOS sem configurar Option como Meta                                                                                                                                                                           |
...
Quando o visualizador de transcrição está aberto (alternado com `Ctrl+O`), estes atalhos estão disponíveis. Em [renderização em tela cheia](/pt/fullscreen), pressione `?` para mostrar o painel de referência de atalho de teclado completo dentro do visualizador. `Ctrl+E` pode ser reatribuído via [`transcript:toggleShowAll`](/pt/keybindings).

| Atalho               | Descrição                                                                                                                                                                                                                                         |
| :------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `?`                  | Alternar o painel de ajuda de atalho de teclado. Requer [renderização em tela cheia](/pt/fullscreen)                                                                                                                                              |
| `{` / `}`            | Pular para o prompt do usuário anterior ou próximo, como movimento de parágrafo vim. Requer [renderização em tela cheia](/pt/fullscreen)                                                                                                          |
