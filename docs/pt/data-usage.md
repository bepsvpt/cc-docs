...
Por padrão, relatório de erros, telemetria e relatório de bugs são desabilitados ao usar Bedrock, Vertex ou Foundry. Pesquisas de qualidade de sessão são a exceção e aparecem independentemente do provedor. Você pode desabilitar todo o tráfego não essencial, incluindo pesquisas, de uma vez definindo `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`. Aqui estão os comportamentos padrão completos:
