...
* **Configuraciones administradas**: Para organizaciones que necesitan control centralizado, Claude Code admite múltiples mecanismos de entrega para configuraciones administradas. Todos utilizan el mismo formato JSON y no pueden ser anulados por configuraciones de usuario o proyecto:
  ...
  * **Basado en archivos**: `managed-settings.json` y `managed-mcp.json` implementados en directorios del sistema:
  ...
  Consulte [configuraciones administradas](/es/permissions#managed-only-settings) y [Configuración de MCP administrada](/es/managed-mcp) para obtener detalles.
...
| `forceLoginMethod`                | Use `claudeai` para restringir el inicio de sesión a cuentas de Claude.ai, `console` para restringir el inicio de sesión a cuentas de Claude Console (facturación de uso de API). Cuando se establece en configuraciones administradas, se bloquean sesiones autenticadas por clave API, `apiKeyHelper`, o un proveedor de terceros al inicio, ya que ninguno de los valores puede satisfacerse sin OAuth de primera parte                                                                                                                                                                                                                                                                                                                                          | `claudeai`                                                                                                                    |
| `forceLoginOrgUUID`               | Requerir que el inicio de sesión pertenezca a una organización específica. Acepta una cadena UUID única, que también preselecciona esa organización durante el inicio de sesión, o una matriz de UUID donde se acepta cualquier organización listada sin preselección. Cuando se establece en configuraciones administradas, el inicio de sesión falla si la cuenta autenticada no pertenece a una organización listada, y sesiones autenticadas por clave API, `apiKeyHelper`, o un proveedor de terceros se bloquean al inicio ya que la pertenencia a la organización no puede verificarse para ellas. Una matriz vacía falla cerrada y bloquea el inicio de sesión con un mensaje de configuración incorrecta                                                   | `"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"` o `["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy"]` |
...
| `mcp`      | Servidores en `~/.claude.json` y `.mcp.json`          | Servidores MCP de plugins, servidores [`managed-mcp.json`](/es/managed-mcp)               |
