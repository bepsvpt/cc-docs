...
| `cleanupPeriodDays`               | Las sesiones inactivas durante más tiempo que este período se eliminan al inicio (predeterminado: 30 días, mínimo 1). Establecer en `0` se rechaza con un error de validación. Para deshabilitar completamente las escrituras de transcripción en modo no interactivo (`-p`), use la bandera `--no-session-persistence` o la opción SDK `persistSession: false`; no hay equivalente en modo interactivo.                                                                                                                                                                                                                                                     | `20`                                                                                                                          |
...
| `disableDeepLinkRegistration`     | Establecer en `"disable"` para evitar que Claude Code registre el controlador de protocolo `claude-cli://` con el sistema operativo al inicio. Los enlaces profundos permiten que herramientas externas abran una sesión de Claude Code con un indicador rellenado previamente a través de `claude-cli://open?q=...`. Útil en entornos donde el registro del controlador de protocolo está restringido o se gestiona por separado                                                                                                                                                                                                                            | `"disable"`                                                                                                                   |
