...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | El modelo a utilizar para todos los [subagents](/es/sub-agents#choose-a-model) y [agent teams](/es/agent-teams). Anula tanto el parámetro `model` por invocación como el frontmatter `model` de la definición del subagent. Establezca en `inherit` para utilizar la resolución de modelo normal en su lugar |
