...
El modo rápido es una configuración de alta velocidad para Claude Opus 4.6, haciendo que el modelo sea 2.5 veces más rápido a un costo más alto por token. Actívalo con `/fast` cuando necesites velocidad para trabajo interactivo como iteración rápida o depuración en vivo, y desactívalo cuando el costo sea más importante que la latencia.
...
Para la mejor eficiencia de costos, habilita el modo rápido al inicio de una sesión en lugar de cambiar a mitad de la conversación. Consulta [entender la compensación de costos](#understand-the-cost-tradeoff) para obtener más detalles.
...
## Entender la compensación de costos
...
<Note>
  El uso del modo rápido se factura directamente al uso adicional, incluso si tienes uso restante en tu plan. Esto significa que los tokens del modo rápido no cuentan contra el uso incluido de tu plan y se cobran a la tarifa del modo rápido desde el primer token.
</Note>
...
Otra opción para desactivar completamente el modo rápido es establecer `CLAUDE_CODE_DISABLE_FAST_MODE=1`. Consulta [Variables de entorno](/es/settings#environment-variables).

### Opción de participación por sesión

De forma predeterminada, el modo rápido persiste entre sesiones: si un usuario habilita el modo rápido, permanece activado en futuras sesiones. Los administradores en planes [Teams](https://claude.com/pricing#team-&-enterprise) o [Enterprise](https://anthropic.com/contact-sales) pueden evitar esto estableciendo `fastModePerSessionOptIn` en `true` en [configuración administrada](/es/settings#settings-files) o [configuración administrada por servidor](/es/server-managed-settings). Esto hace que cada sesión comience con el modo rápido desactivado, requiriendo que los usuarios lo habiliten explícitamente con `/fast`.
...
1. El modo rápido automáticamente vuelve al Opus 4.6 estándar
