...
Il timeout per server è un limite rigido di wall-clock per chiamata dello strumento, e le notifiche di progresso dal server non lo estendono. I valori inferiori a 1000 sono arrotondati a un secondo. Per i server HTTP e SSE, il budget del primo byte per richiesta fetch ha un minimo di 60 secondi indipendentemente da questo valore, quindi solo il watchdog della chiamata dello strumento onora valori più piccoli.
