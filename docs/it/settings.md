...
| `cleanupPeriodDays`               | Le sessioni inattive per un periodo più lungo di questo vengono eliminate all'avvio (predefinito: 30 giorni).<br /><br />L'impostazione a `0` elimina tutti i trascritti esistenti all'avvio e disabilita completamente la persistenza della sessione. Nessun nuovo file `.jsonl` viene scritto, `/resume` non mostra conversazioni, e gli hook ricevono un `transcript_path` vuoto. | `20`                                                                    |
...
| `includeGitInstructions`          | Includi le istruzioni integrate del flusso di lavoro di commit e PR nel prompt di sistema di Claude (predefinito: `true`). Imposta a `false` per rimuovere queste istruzioni, ad esempio quando utilizzi le tue skill di flusso di lavoro git. La variabile di ambiente `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` ha la precedenza su questa impostazione quando impostata              | `false`                                                                 |
...
| `showTurnDuration`                | Mostra i messaggi di durata del turno dopo le risposte (ad es., "Cooked for 1m 6s"). Imposta a `false` per nascondere questi messaggi                                                                                                                                                                                                                                                | `true`                                                                  |
...
| `defaultMode`                  | [Modalità di permesso](/it/permissions#permission-modes) predefinita quando si apre Claude Code                                                                                                                                                                                        | `"acceptEdits"`                                                        |
| `disableBypassPermissionsMode` | Imposta a `"disable"` per prevenire l'attivazione della modalità `bypassPermissions`. Questo disabilita il flag della riga di comando `--dangerously-skip-permissions`. Vedi [impostazioni gestite](/it/permissions#managed-only-settings)                                             | `"disable"`                                                            |
