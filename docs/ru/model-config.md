...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | Модель для использования для всех [subagents](/ru/sub-agents#choose-a-model). Переопределяет как параметр `model` для каждого вызова, так и frontmatter `model` определения subagent |
