...
Начиная с версии v2.1.144, `/model` применяется только к текущему сеансу и не записывается в параметры. Чтобы сохранить ваш выбор в качестве значения по умолчанию для новых сеансов, нажмите `d` на выделенной строке в средстве выбора, которое записывает поле `model` в ваши пользовательские параметры. Управляемые параметры имеют приоритет и переприменяются при следующем запуске.

Флаг `--model` и переменная окружения `ANTHROPIC_MODEL` также применяются только к сеансу, с которым вы их запускаете. Чтобы запустить разные модели в разных терминалах одновременно, запустите каждый с собственным флагом `--model` вместо переключения с помощью `/model`.
...
* **Параметры**: установите `effortLevel` в `low`, `medium`, `high` или `xhigh` в файле параметров. `max` является [только для сеанса](#adjust-effort-level) и не принимается здесь
...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | Модель для использования для всех [subagents](/ru/sub-agents#choose-a-model) и [agent teams](/ru/agent-teams). Переопределяет как параметр `model` для каждого вызова, так и frontmatter `model` определения subagent. Установите на `inherit` для использования обычного разрешения модели вместо этого |
...
Суффикс `[1m]` применяет контекстное окно 1M ко всему использованию этого псевдонима, включая `opusplan`.

* Claude Code удаляет суффикс перед отправкой идентификатора модели вашему поставщику.
* Добавляйте `[1m]` только когда базовая модель [поддерживает контекст 1M](https://platform.claude.com/docs/en/build-with-claude/context-windows#1m-token-context-window).
* Суффикс читается для каждой переменной, а не для каждой модели. На Bedrock, Vertex и Foundry идентификатор модели без `[1m]` в одной переменной использует контекст 200K, даже если другая переменная устанавливает ту же модель с суффиксом.
...
Claude Code автоматически использует [кэширование подсказок](/ru/prompt-caching) для оптимизации производительности и снижения затрат. Вы можете отключить кэширование подсказок глобально или для конкретных уровней моделей:
...
Чтобы изменить TTL кэша или узнать, что вызывает промах кэша, см. [Как Claude Code использует кэширование подсказок](/ru/prompt-caching).
