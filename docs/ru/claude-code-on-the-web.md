...
<Warning>
  Если ваш репозиторий использует автоматизацию, запускаемую комментариями, такую как Atlantis, Terraform Cloud или пользовательские GitHub Actions, которые запускаются на событиях `issue_comment`, имейте в виду, что ответы Claude могут запустить эти рабочие процессы. Проверьте автоматизацию вашего репозитория перед включением auto-fix и рассмотрите отключение auto-fix для репозиториев, где комментарий PR может развернуть инфраструктуру или запустить привилегированные операции.
</Warning>
