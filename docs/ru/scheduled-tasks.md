...
<Note>
  Встроенная подсказка обслуживания доступна не всем и не поддерживается на Bedrock, Vertex AI и Microsoft Foundry. Где она не активна, `/loop` без подсказки выводит сообщение об использовании вместо этого.
</Note>
...
<Note>
  `loop.md` следует той же доступности, что и [встроенная подсказка обслуживания](#run-the-built-in-maintenance-prompt). Где подсказка обслуживания не активна, `/loop` без подсказки выводит сообщение об использовании и файл не читается.
</Note>
