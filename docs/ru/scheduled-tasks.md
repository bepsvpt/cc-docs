...
Запланированные задачи позволяют Claude автоматически повторно запускать подсказку через определённый интервал. Используйте их для опроса развёртывания, контроля PR, проверки долгоживущей сборки или напоминания себе о чём-то позже в сеансе.
...
Встроенный навык `/loop` [bundled skill](/ru/skills#bundled-skills) — это самый быстрый способ запланировать повторяющуюся подсказку. Передайте необязательный интервал и подсказку, и Claude установит задание cron, которое будет срабатывать в фоне, пока сеанс остаётся открытым.
...
Запланированная подсказка может быть сама по себе командой или вызовом навыка. Это полезно для повторного запуска уже упакованного рабочего процесса.

```text  theme={null}
/loop 20m /review-pr 1234
```

Каждый раз, когда задание срабатывает, Claude запускает `/review-pr 1234` так, как если бы вы его набрали.
...
| :----------- | :----------------------------------------------------------------------------------------------------------------------------------- |
| `CronCreate` | Запланировать новую задачу. Принимает 5-полевое выражение cron, подсказку для запуска и повторяется ли она или срабатывает один раз. |
...
Чтобы избежать того, чтобы каждый сеанс обращался к API в один и тот же момент по стене, планировщик добавляет небольшое детерминированное смещение к времени срабатывания:
...
Установите `CLAUDE_CODE_DISABLE_CRON=1` в вашей среде, чтобы полностью отключить планировщик. Инструменты cron и `/loop` становятся недоступными, и любые уже запланированные задачи перестают срабатывать. См. [Переменные окружения](/ru/settings#environment-variables) для полного списка флагов отключения.
