...
| `strict`      | boolean | Контролирует, является ли `plugin.json` авторитетом для определений компонентов (по умолчанию: true). См. [Строгий режим](#strict-mode) ниже. |
...
| Поле  | Тип    | Описание                                                                           |
| :---- | :----- | :--------------------------------------------------------------------------------- |
| `url` | string | Обязательно. Полный URL репозитория Git (должен заканчиваться на `.git`)           |
| `ref` | string | Опционально. Ветка или тег Git (по умолчанию ветка по умолчанию репозитория)       |
| `sha` | string | Опционально. Полный 40-символьный SHA коммита Git для закрепления на точной версии |
...
* **`strict: false`**: Поскольку это установлено на false, плагину не нужен собственный `plugin.json`. Запись marketplace определяет все. См. [Строгий режим](#strict-mode) ниже.
...
| `plugins[0].source: Path traversal not allowed`   | Путь источника содержит `..` | Используйте пути относительно корня marketplace без `..`                   |
