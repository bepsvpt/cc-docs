...
| `url`              | object                               | `url` (должен заканчиваться на .git), `ref?`, `sha?` | Источник URL Git                                                                                                     |
...
| `pip`              | object                               | `package`, `version?`, `registry?`                   | Установлено через pip                                                                                                |
...
* **`${CLAUDE_PLUGIN_ROOT}`**: Используйте эту переменную в hooks и конфигурациях MCP server для ссылки на файлы в каталоге установки плагина. Это необходимо, потому что плагины копируются в место кэша при установке.
...
Разрешение всех marketplace с внутреннего сервера Git с использованием сопоставления шаблонов регулярных выражений на хосте:
...
Запустите `claude plugin validate .` или `/plugin validate .` из каталога вашего marketplace, чтобы проверить наличие проблем. Общие ошибки:
...
| `Invalid JSON syntax: Unexpected token...`        | Ошибка синтаксиса JSON       | Проверьте отсутствующие запятые, лишние запятые или неквотированные строки                          |
