...
> Откройте решения для распространённых проблем с установкой и использованием Claude Code.

## Устранение проблем с установкой

<Tip>
  Если вы предпочитаете избежать терминала, [приложение Claude Code Desktop](/ru/desktop-quickstart) позволяет вам установить и использовать Claude Code через графический интерфейс. Загрузите его для [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) или [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs) и начните кодировать без какой-либо настройки командной строки.
</Tip>
...
| `Failed to fetch version` или не удаётся достичь сервера загрузки | [Проверьте параметры сети и прокси](#check-network-connectivity)                                                   |
...
Установщик загружает из `storage.googleapis.com`. Убедитесь, что вы можете его достичь:
...
Если установка прошла успешно, но вы получаете ошибку `command not found` или `not recognized` при запуске `claude`, каталог установки не находится в вашей PATH. Ваша оболочка ищет программы в каталогах, указанных в PATH, и установщик размещает `claude` в `~/.local/bin/claude` на macOS/Linux или `%USERPROFILE%\.local\bin\claude.exe` на Windows.

Проверьте, находится ли каталог установки в вашей PATH, перечислив записи PATH и отфильтровав `local/bin`:
...
<Tabs>
  <Tab title="macOS/Linux">
    ...
    Если нет вывода, каталог отсутствует. Добавьте его в конфигурацию вашей оболочки:
  ...
  <Tab title="Windows PowerShell">
    ...
    Если нет вывода, добавьте каталог установки в вашу User PATH:
  ...
  <Tab title="Windows CMD">
    ...
    Если нет вывода, откройте Параметры системы, перейдите в Переменные среды и добавьте `%USERPROFILE%\.local\bin` в вашу переменную User PATH. Перезагрузите ваш терминал.
...
Если вы найдёте несколько установок, оставьте только одну. Собственная установка в `~/.local/bin/claude` рекомендуется. Удалите любые дополнительные установки:
...
Установщику требуется доступ на запись к `~/.local/bin/` и `~/.claude/`. Если установка не удаётся с ошибками разрешений, проверьте, доступны ли эти каталоги для записи:
...
Если какой-либо каталог недоступен для записи, создайте каталог установки и установите вашего пользователя в качестве владельца:
...
На Linux проверьте наличие отсутствующих общих библиотек. Если `ldd` показывает отсутствующие библиотеки, вам может потребоваться установить системные пакеты. На Alpine Linux и других дистрибутивах на основе musl см. [Настройка Alpine Linux](/ru/setup#alpine-linux-and-musl-based-distributions).

```bash  theme={null}
ldd $(which claude) | grep "not found"
```

Выполните быструю проверку работоспособности, что двоичный файл может выполняться:

```bash  theme={null}
claude --version
```

## Распространённые проблемы с установкой

Это наиболее часто встречающиеся проблемы с установкой и их решения.
...
В противном случае это может произойти из-за проблем с сетью, региональной маршрутизации или временного сбоя в обслуживании.
...
Установка завершена, но `claude` не работает. Точная ошибка варьируется в зависимости от платформы:
...
Это означает, что каталог установки не находится в пути поиска вашей оболочки. См. [Проверьте вашу PATH](#verify-your-path) для исправления на каждой платформе.

### `curl: (56) Failure writing output to destination`

Команда `curl ... | bash` загружает скрипт и передаёт его непосредственно Bash для выполнения, используя конвейер (`|`). Эта ошибка означает, что соединение разорвалось до завершения загрузки скрипта. Распространённые причины включают сетевые перебои, блокировку загрузки в середине потока или ограничения системных ресурсов.
...
3. **Проверьте наличие помех прокси или брандмауэра**: корпоративные прокси, выполняющие проверку TLS, могут вызвать эти ошибки, включая `unable to get local issuer certificate`. Установите `NODE_EXTRA_CA_CERTS` на ваш пакет корпоративного сертификата CA:
   ```bash  theme={null}
...
1. **Установите рабочий каталог** перед запуском установщика. При запуске из `/` установщик сканирует всю файловую систему, что вызывает чрезмерное использование памяти. Установка `WORKDIR` ограничивает сканирование небольшим каталогом:
...
Если вы установили более старую версию Claude Desktop, она может зарегистрировать `Claude.exe` в каталоге `WindowsApps`, который имеет приоритет PATH над Claude Code CLI. Запуск `claude` открывает приложение Desktop вместо CLI.

Обновите Claude Desktop до последней версии, чтобы исправить эту проблему.

### Windows: "Claude Code on Windows requires git-bash"

Claude Code на собственном Windows требует [Git for Windows](https://git-scm.com/downloads/win), который включает Git Bash.
...
Если ваш Git установлен в другом месте, найдите путь, запустив `where.exe git` в PowerShell и используйте путь `bin\bash.exe` из этого каталога.

### Linux: неправильный вариант двоичного файла установлен (несоответствие musl/glibc)

Если вы видите ошибки об отсутствующих общих библиотеках, таких как `libstdc++.so.6` или `libgcc_s.so.1` после установки, установщик мог загрузить неправильный вариант двоичного файла для вашей системы.
...
1. **Проверьте, какой libc использует ваша система**:
   ...
   ldd /bin/ls | head -1
   ```
   Если он показывает `linux-vdso.so` или ссылки на `/lib/x86_64-linux-gnu/`, вы находитесь на glibc. Если он показывает `musl`, вы находитесь на musl.
...
3. **Если вы действительно находитесь на musl** (Alpine Linux), установите необходимые пакеты:
   ```bash  theme={null}
...
Если установщик выводит `Illegal instruction` вместо сообщения OOM `Killed`, загруженный двоичный файл не соответствует архитектуре вашего процессора. Это обычно происходит на серверах ARM, которые получают двоичный файл x86, или на более старых процессорах, которым не хватает необходимых наборов инструкций.
...
2. **Обновите macOS**, если вы находитесь на более старой версии. Двоичный файл использует команды загрузки, которые более старые версии macOS не поддерживают.
...
Если nvm правильно загружен, но пути Windows всё ещё имеют приоритет, вы можете явно добавить ваши пути Linux в PATH в конфигурацию вашей оболочки:
...
<Warning>
  Избегайте отключения импорта Windows PATH через `appendWindowsPath = false`, так как это нарушает возможность вызова исполняемых файлов Windows из WSL. Аналогично, избегайте удаления Node.js из Windows, если вы используете его для разработки Windows.
</Warning>
...
WSL1 не поддерживает sandboxing. Если вы видите "Sandboxing requires WSL2", вам нужно обновиться до WSL2 или запустить Claude Code без sandboxing.

### Ошибки разрешений во время установки

Если собственный установщик не удаётся с ошибками разрешений, целевой каталог может быть недоступен для записи. См. [Проверьте разрешения каталога](#check-directory-permissions).
...
Эти разделы рассматривают сбои входа, проблемы с токенами и поведение подсказок разрешений.

### Повторяющиеся подсказки разрешений

Если вы обнаружите, что постоянно одобряете одни и те же команды, вы можете разрешить определённым инструментам работать без одобрения, используя команду `/permissions`. См. [документацию Permissions](/ru/permissions#manage-permissions).
...
3. Перезагрузитесь с помощью `claude` и завершите процесс аутентификации снова

Если браузер не открывается автоматически во время входа, нажмите `c`, чтобы скопировать URL OAuth в буфер обмена, затем вставьте его в ваш браузер вручную.
...
Или скопируйте URL вручную: когда появится подсказка входа, нажмите `c`, чтобы скопировать URL OAuth, затем вставьте его в ваш браузер Windows.

### "Not logged in" или token expired

Если Claude Code предлагает вам войти снова после сеанса, ваш OAuth токен может истечь.
...
| Управляемые параметры         | [Управляемые параметры](/ru/settings#settings-files) (управляемые сервером, политики MDM/OS-уровня или на основе файлов) |

На Windows `~` относится к вашему домашнему каталогу пользователя, например `C:\Users\YourName`.
...
Затем установите `USE_BUILTIN_RIPGREP=0` в вашем [окружении](/ru/settings#environment-variables).
...
1. **Отправляйте более конкретные поиски**: уменьшите количество файлов, которые ищутся, указав каталоги или типы файлов: "Search for JWT validation logic in the auth-service package" или "Find use of md5 hash in JS files".
...
3. **Используйте собственный Windows вместо этого**: рассмотрите запуск Claude Code в собственном Windows вместо WSL для лучшей производительности файловой системы.
...
WSL2 использует сетевой адрес NAT по умолчанию, что может помешать обнаружению IDE. У вас есть два варианта:
...
Для дополнительных советов по конфигурации JetBrains см. [руководство JetBrains IDE](/ru/jetbrains#plugin-settings).
...
* Тип окружения: собственный Windows (Git Bash) или WSL1/WSL2
...
### Клавиша Escape не работает в терминалах JetBrains IDE
...
2. **Используйте инструменты форматирования**: установите hooks для запуска форматировщиков markdown, таких как `prettier` или пользовательские скрипты форматирования на сгенерированных файлах markdown.

3. **Укажите предпочтения форматирования**: включите требования форматирования в ваши подсказки или файлы [memory](/ru/memory) проекта.
