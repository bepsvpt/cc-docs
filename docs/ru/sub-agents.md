...
<Note>
  Forked subagents экспериментальны и требуют Claude Code v2.1.117 или позже. Поведение и конфигурация могут измениться в будущих выпусках. Включите их, установив переменную окружения [`CLAUDE_CODE_FORK_SUBAGENT`](/ru/env-vars) на `1`.
</Note>
...
Running forks появляются в панели ниже входа приглашения, с одной строкой для основной сессии и одной для каждого fork. Используйте эти клавиши для взаимодействия с панелью:
...
Fork mode работает только в интерактивных сессиях. Он отключен в [non-interactive mode](/ru/headless), который включает Agent SDK. Fork не может порождать дальнейшие forks.
