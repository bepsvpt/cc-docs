...
**Новичок в Claude Code?** Начните с [CLAUDE.md](/ru/memory) для соглашений проекта, затем добавляйте другие расширения [по мере возникновения конкретных триггеров](#build-your-setup-over-time).
...
* **[Code intelligence](/ru/tools-reference#lsp-tool-behavior)** подключает Claude к языковому серверу для навигации на уровне символов и ошибок типов в реальном времени
...
* **[Hooks](/ru/hooks-guide)** срабатывают при событиях жизненного цикла и могут запустить скрипт, HTTP запрос, prompt или subagent
...
| **[Code intelligence](/ru/tools-reference#lsp-tool-behavior)** | Навигация языкового сервера и диагностика                                   | Типизированные языки, большие кодовые базы, где grep медленен или неточен                | Перейти к определению символа вместо чтения всего файла                                                        |
...
| Триггер                                                                                  | Добавить                                                                                       |
| :--------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- |
| Claude дважды неправильно применяет соглашение или команду                               | Добавьте в [CLAUDE.md](/ru/memory)                                                             |
| Вы постоянно вводите один и тот же prompt для начала задачи                              | Сохраните как вызываемый пользователем [skill](/ru/skills)                                     |
| Вы вставляете один и тот же playbook или многошаговую процедуру в чат в третий раз       | Захватите как [skill](/ru/skills)                                                              |
| Вы постоянно копируете данные из вкладки браузера, которую Claude не видит               | Подключите эту систему как [MCP server](/ru/mcp)                                               |
| Claude читает много файлов, чтобы найти, где определён или используется символ           | Установите [плагин code intelligence](/ru/discover-plugins#code-intelligence) для вашего языка |
| Побочная задача заполняет ваш разговор выводом, на который вы больше не будете ссылаться | Маршрутизируйте через [subagent](/ru/sub-agents)                                               |
| Вы хотите, чтобы что-то происходило каждый раз без запроса                               | Напишите [hook](/ru/hooks-guide)                                                               |
| Второй репозиторий нуждается в одной и той же настройке                                  | Упакуйте как [plugin](/ru/plugins)                                                             |
...
<Tabs>
  <Tab title="Skill vs Subagent">
    ...
    * **Subagents** — это изолированные работники, которые работают отдельно от вашего основного разговора
    ...
    | **[Влияние на окно контекста](/ru/context-window)** | Добавляется в ваше основное окно                         | Использует отдельное окно со своими входными и выходными токенами                      |
  ...
  <Tab title="Hook vs Skill">
    ...
    | **Запущено**            | [События жизненного цикла](/ru/hooks#hook-events) такие как `PostToolUse` или `SessionStart` | Вы вводите `/<name>`, или Claude сопоставляет описание с вашей задачей                      |
...
<Tabs>
  ...
  <Tab title="Skills">
    Skills — это дополнительные возможности в наборе инструментов Claude. Они могут быть справочным материалом (например, руководство по стилю API) или вызываемыми рабочими процессами, которые вы запускаете с помощью `/<name>` (например, `/deploy`). Claude Code поставляется с [встроенными skills](/ru/commands) типа `/code-review`, `/batch` и `/debug`, которые работают из коробки. Вы также можете создавать свои собственные. Claude использует skills, когда это уместно, или вы можете вызвать один напрямую.
  ...
  <Tab title="MCP servers">
    ...
    **Стоимость контекста:** [Tool search](/ru/mcp#scale-with-mcp-tool-search) включён по умолчанию, поэтому неиспользуемые MCP инструменты потребляют минимальный контекст.
  ...
  <Tab title="Code intelligence">
    ...
    **Стоимость контекста:** Низкая. Поиск символов часто заменяет широкое чтение файлов, поэтому общее использование контекста может снизиться.
    ...
    <Tip>Инструмент LSP неактивен до тех пор, пока вы не установите [плагин code intelligence](/ru/discover-plugins#code-intelligence) для вашего языка.</Tip>
  </Tab>
  ...
  <Tab title="Subagents">
    ...
    * CLAUDE.md и статус git, за исключением встроенных агентов Explore и Plan [которые опускают оба](/ru/sub-agents#what-loads-at-startup)
  ...
  <Tab title="Hooks">
    **Когда:** При срабатывании. Hooks срабатывают при определённых событиях жизненного цикла, таких как выполнение инструмента, границы сессии, отправка prompt, запросы разрешений и компактирование. Подробный список см. в разделе [Hooks](/ru/hooks).
