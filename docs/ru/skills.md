...
Флаг `--add-dir` [предоставляет доступ к файлам](/ru/permissions#additional-directories-grant-file-access-not-configuration) скорее, чем конфигурацию обнаружения, но skills — это исключение: `.claude/skills/` в добавленном каталоге загружается автоматически и подхватывается обнаружением живых изменений, поэтому вы можете редактировать эти skills во время сессии без перезагрузки.

Другая конфигурация `.claude/`, такая как subagents, команды и стили выходных данных, не загружается из дополнительных каталогов. См. [таблицу исключений](/ru/permissions#additional-directories-grant-file-access-not-configuration) для полного списка того, что загружается и что не загружается, и рекомендуемые способы совместного использования конфигурации между проектами.
...
| `description`              | Рекомендуется | Что делает skill и когда его использовать. Claude использует это, чтобы решить, когда применять skill. Если опущено, использует первый абзац содержимого markdown. Описания длиннее 250 символов усекаются в списке skills для уменьшения использования контекста.                                                                             |
...
| `allowed-tools`            | Нет           | Инструменты, которые Claude может использовать без запроса разрешения, когда этот skill активен. Принимает строку, разделённую пробелами, или список YAML.                                                                                                                                                                                     |
| `model`                    | Нет           | Модель для использования, когда этот skill активен.                                                                                                                                                                                                                                                                                            |
| `effort`                   | Нет           | [Уровень усилий](/ru/model-config#adjust-effort-level) при активном этом skill. Переопределяет уровень усилий сессии. По умолчанию: наследует из сессии. Опции: `low`, `medium`, `high`, `max` (только Opus 4.6).                                                                                                                              |
...
| `shell`                    | Нет           | Shell для использования в блоках `` !`command` `` в этом skill. Принимает `bash` (по умолчанию) или `powershell`. Установка `powershell` запускает встроенные команды shell через PowerShell на Windows. Требует `CLAUDE_CODE_USE_POWERSHELL_TOOL=1`.                                                                                          |
...
Чтобы повысить лимит, установите переменную окружения `SLASH_COMMAND_TOOL_CHAR_BUDGET`. Или обрежьте описания в источнике: поместите ключевой вариант использования в начало, так как каждая запись ограничена 250 символами независимо от бюджета.
