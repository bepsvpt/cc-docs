...
<Steps>
  ...
  <Step title="Получите контекст о взаимодействии компонентов">
    ```text  theme={null}
...
Предположим, вы столкнулись с сообщением об ошибке и вам нужно найти и исправить её источник.
...
<Tip>Нажмите `Ctrl+G` для открытия плана в вашем текстовом редакторе по умолчанию, где вы можете отредактировать его напрямую перед тем, как Claude продолжит.</Tip>
...
## Использование расширенного мышления (режим thinking)

[Расширенное мышление](https://platform.claude.com/docs/en/build-with-claude/extended-thinking) включено по умолчанию, предоставляя Claude пространство для пошагового рассуждения о сложных проблемах перед ответом. Это рассуждение видно в режиме verbose, который вы можете переключить с помощью `Ctrl+O`.

Кроме того, Opus 4.6 вводит адаптивное рассуждение: вместо фиксированного бюджета токенов thinking, модель динамически распределяет thinking на основе вашего параметра [уровня усилий](/ru/model-config#adjust-effort-level). Расширенное мышление и адаптивное рассуждение работают вместе, чтобы дать вам контроль над тем, насколько глубоко Claude рассуждает перед ответом.
...
<Note>
  Фразы вроде "think", "think hard" и "think more" интерпретируются как обычные инструкции подсказки и не распределяют токены thinking.
</Note>

### Настройка режима thinking

Thinking включено по умолчанию, но вы можете его отрегулировать или отключить.

| Область                              | Как настроить                                                                                            | Подробности                                                                                                                                                                             |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Уровень усилий**                   | Отрегулируйте в `/model` или установите [`CLAUDE_CODE_EFFORT_LEVEL`](/ru/settings#environment-variables) | Контролируйте глубину thinking для Opus 4.6 и Sonnet 4.6: low, medium, high. См. [Отрегулируйте уровень усилий](/ru/model-config#adjust-effort-level)                                   |
...
| **Сочетание клавиш переключения**    | Нажмите `Option+T` (macOS) или `Alt+T` (Windows/Linux)                                                   | Переключите thinking включен/выключен для текущего сеанса (все модели). Может потребоваться [конфигурация терминала](/ru/terminal-config) для включения сочетаний клавиш Option         |
| **Глобальное значение по умолчанию** | Используйте `/config` для переключения режима thinking                                                   | Устанавливает ваше значение по умолчанию для всех проектов (все модели).<br />Сохраняется как `alwaysThinkingEnabled` в `~/.claude/settings.json`                                       |
| **Ограничить бюджет токенов**        | Установите переменную окружения [`MAX_THINKING_TOKENS`](/ru/settings#environment-variables)              | Ограничьте бюджет thinking определённым количеством токенов (игнорируется на Opus 4.6, если не установлено на 0). Пример: `export MAX_THINKING_TOKENS=10000`                            |

Чтобы просмотреть процесс thinking Claude, нажмите `Ctrl+O` для переключения режима verbose и посмотрите внутреннее рассуждение, отображаемое как серый курсивный текст.

### Как работает расширенное мышление

Расширенное мышление контролирует, сколько внутреннего рассуждения Claude выполняет перед ответом. Больше thinking предоставляет больше пространства для изучения решений, анализа граничных случаев и самокоррекции ошибок.

**С Opus 4.6**, thinking использует адаптивное рассуждение: модель динамически распределяет токены thinking на основе [уровня усилий](/ru/model-config#adjust-effort-level), который вы выбираете (low, medium, high). Это рекомендуемый способ настройки компромисса между скоростью и глубиной рассуждения.

**С другими моделями**, thinking использует фиксированный бюджет до 31 999 токенов из вашего выходного бюджета. Вы можете ограничить это с помощью переменной окружения [`MAX_THINKING_TOKENS`](/ru/settings#environment-variables), или отключить thinking полностью через `/config` или переключение `Option+T`/`Alt+T`.

`MAX_THINKING_TOKENS` игнорируется на Opus 4.6 и Sonnet 4.6, так как адаптивное рассуждение контролирует глубину thinking вместо этого. Одно исключение: установка `MAX_THINKING_TOKENS=0` всё ещё полностью отключает thinking на любой модели. Чтобы отключить адаптивное thinking и вернуться к фиксированному бюджету thinking, установите `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`. См. [переменные окружения](/ru/settings#environment-variables).

<Warning>
  Вам выставляется счёт за все использованные токены thinking, даже если модели Claude 4 показывают сокращённое thinking
</Warning>
...
| `Enter`          | Выбрать и возобновить выделенный сеанс                 |
| `P`              | Предпросмотр содержимого сеанса                        |
| `R`              | Переименовать выделенный сеанс                         |
...
Worktrees создаются в `<repo>/.claude/worktrees/<name>` и ветвятся от ветки удалённого репозитория по умолчанию. Ветка worktree названа `worktree-<name>`.
...
Для большего контроля над расположением worktree и конфигурацией ветки создавайте worktrees напрямую с Git. Это полезно, когда вам нужно проверить конкретную существующую ветку или разместить worktree вне репозитория.
...
<Steps>
  <Step title="Откройте меню hooks">
    Введите `/hooks` и выберите `Notification` из списка событий.
  </Step>

  <Step title="Настройте matcher">
    Выберите `+ Match all (no filter)` для срабатывания на всех типах уведомлений. Чтобы уведомлять только для конкретных событий, выберите `+ Add new matcher…` и введите одно из этих значений:
  ...
  <Step title="Добавьте вашу команду уведомления">
    Выберите `+ Add new hook…` и введите команду для вашей ОС:
    ...
    <Tabs>
      ...
      <Tab title="Windows (PowerShell)">
        Использует PowerShell для отображения собственного окна сообщения через .NET Windows Forms:
  ...
  <Step title="Сохраните в пользовательские параметры">
    Выберите `User settings` для применения уведомления ко всем вашим проектам.
  </Step>
</Steps>

Для полного пошагового руководства с примерами конфигурации JSON см. [Автоматизация рабочих процессов с помощью hooks](/ru/hooks-guide#get-notified-when-claude-needs-input). Для полной схемы событий и типов уведомлений см. [справочник Notification](/ru/hooks#notification).
...
Предположим, вы хотите использовать Claude Code как linter или code reviewer.
...
<Steps>
  <Step title="Используйте текстовый формат (по умолчанию)">
    ...
    Это выводит просто простой текстовый ответ Claude (поведение по умолчанию).
  </Step>
  ...
  <Step title="Используйте потоковый JSON формат">
    ...
    Это выводит серию JSON объектов в реальном времени по мере обработки Claude запроса. Каждое сообщение является действительным JSON объектом, но весь вывод не является действительным JSON при конкатенации.
  </Step>
</Steps>
