...
<Steps>
  ...
  <Step title="Опционально сузьте matcher">
    ...
    | `elicitation_dialog`   | Сервер MCP открывает форму запроса информации                    |
    | `elicitation_complete` | Форма запроса информации MCP отправлена или отклонена            |
    | `elicitation_response` | Ответ на запрос информации MCP отправляется обратно на сервер    |
  </Step>
