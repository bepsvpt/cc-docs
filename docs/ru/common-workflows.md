...
На этой странице собраны короткие рецепты для повседневной разработки. Для получения более высокоуровневого руководства по подсказкам и управлению контекстом см. [Лучшие практики](/ru/best-practices).
...
Когда вы создаёте PR с помощью `gh pr create`, сеанс автоматически связывается с этим PR. Чтобы вернуться к нему позже, запустите `claude --from-pr <number>` или вставьте URL PR в средство выбора [`/resume`](/ru/sessions#use-the-session-picker).
...
| :------------------------------------------------------------------- | :--------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Routines](/ru/routines)                                             | Инфраструктура, управляемая Anthropic    | Задачи, которые должны выполняться даже когда ваш компьютер выключен. Также может срабатывать на вызовы API или события GitHub в дополнение к расписанию. Настройте на [claude.ai/code/routines](https://claude.ai/code/routines). |
| [Запланированные задачи рабочего стола](/ru/desktop-scheduled-tasks) | Ваша машина, через настольное приложение | Задачи, которым нужен прямой доступ к локальным файлам, инструментам или незафиксированным изменениям.                                                                                                                             |
| [GitHub Actions](/ru/github-actions)                                 | Ваш конвейер CI                          | Задачи, связанные с событиями репозитория, такими как открытые PR, или расписания cron, которые должны находиться рядом с конфигурацией рабочего процесса.                                                                         |
| [`/loop`](/ru/scheduled-tasks)                                       | Текущий сеанс CLI                        | Быстрый опрос во время открытого сеанса. Задачи отменяются при начале нового разговора; `--resume` и `--continue` восстанавливают неистекшие.                                                                                      |
...
Вы также можете нажать `Shift+Tab` во время сеанса для переключения в режим плана. См. [Plan Mode](/ru/permission-modes#analyze-before-you-edit-with-plan-mode) для потока одобрения и редактирования плана в вашем текстовом редакторе.
...
Subagent читает файлы в своём собственном контекстном окне и сообщает сводку. См. [Subagents](/ru/sub-agents) для определения пользовательских агентов с их собственными инструментами и подсказками.
...
См. [Неинтерактивный режим](/ru/headless) для форматов вывода, флагов разрешений и паттернов fan-out.
...
<CardGroup cols={2}>
  ...
  <Card title="Управление сеансами" icon="rotate-left" href="/ru/sessions">
    Возобновление, именование и ветвление разговоров
  </Card>

  <Card title="Worktrees" icon="code-branch" href="/ru/worktrees">
    Запуск изолированных параллельных сеансов
  </Card>

  <Card title="Расширение Claude Code" icon="puzzle-piece" href="/ru/features-overview">
