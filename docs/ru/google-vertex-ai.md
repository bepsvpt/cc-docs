...
[Поиск инструментов MCP](/ru/mcp#scale-with-mcp-tool-search) отключен по умолчанию на Vertex AI, так как конечная точка не принимает требуемый бета-заголовок. Вместо этого все определения инструментов MCP загружаются заранее. Чтобы включить эту функцию, установите `ENABLE_TOOL_SEARCH=true`.
