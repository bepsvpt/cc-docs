...
<Note>
  Дополнительные атрибуты, содержащие содержимое, такие как `new_context`, `system_prompt_preview`, `tool_input` и `response.model_output`, выдаются только при активной детальной бета-трассировке. Они не являются частью стабильной схемы span.
</Note>
...
Логируется, когда режим разрешений изменяется, например при циклировании Shift+Tab, выходе из режима Plan Mode или проверке gate автоматического режима.
...
Логируется, когда навык вызывается.
...
* `status_code`: HTTP код состояния в виде строки
