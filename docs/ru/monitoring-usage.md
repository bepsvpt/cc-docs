...
| `OTEL_LOG_TOOL_DETAILS`                             | Включить логирование имен MCP сервера/инструмента и имен навыков в событиях инструментов (по умолчанию: отключено)                    | `1` для включения                    |
...
* `tool_parameters`: JSON строка, содержащая параметры, специфичные для инструмента (когда доступно)
  ...
  * Для инструментов MCP (когда `OTEL_LOG_TOOL_DETAILS=1`): включает `mcp_server_name`, `mcp_tool_name`
  * Для инструмента Skill (когда `OTEL_LOG_TOOL_DETAILS=1`): включает `skill_name`
...
* Содержимое файлов в исходном виде и фрагменты кода не включаются в метрики или события. События выполнения инструментов включают команды bash и пути к файлам в поле `tool_parameters`, которое может содержать конфиденциальные значения. Если ваши команды могут включать секреты, настройте ваш бэкенд телеметрии для фильтрации или редактирования `tool_parameters`
...
* Имена MCP сервера/инструмента и имена навыков не логируются по умолчанию, потому что они могут раскрыть конфигурации, специфичные для пользователя. Чтобы включить их, установите `OTEL_LOG_TOOL_DETAILS=1`
