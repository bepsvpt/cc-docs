...
Расширенное мышление включено по умолчанию с бюджетом 31,999 токенов, потому что оно значительно улучшает производительность на сложных задачах планирования и рассуждения. Однако токены мышления выставляются как выходные токены, поэтому для более простых задач, где глубокое рассуждение не требуется, вы можете снизить затраты, понизив [уровень усилий](/ru/model-config#adjust-effort-level) в `/model` для Opus 4.6, отключив мышление в `/config` или понизив бюджет (например, `MAX_THINKING_TOKENS=8000`).
