...
После установки запустите Claude, войдите в систему и нажмите на вкладку **Code**. Полное пошаговое руководство вашего первого сеанса см. в [Руководстве по началу работы](/ru/desktop-quickstart).

Desktop добавляет эти возможности к стандартному опыту Claude Code:

* [Параллельные сеансы](#work-in-parallel-with-sessions) с автоматической изоляцией Git worktree
* [Макет с перетаскиванием](#arrange-your-workspace) с интегрированным терминалом, редактором файлов и панелью предпросмотра
* [Боковые чаты](#ask-a-side-question-without-derailing-the-session), которые ветвятся без влияния на основной поток
* [Визуальный просмотр различий](#review-changes-with-diff-view) с встроенными комментариями
* [Живой предпросмотр приложения](#preview-your-app) с dev-серверами, HTML-файлами и PDF
* [Использование компьютера](#let-claude-use-your-computer) для открытия приложений и управления экраном на macOS и Windows
* [Мониторинг GitHub PR](#monitor-pull-request-status) с автоисправлением, автослиянием и автоархивированием
* [Dispatch](#sessions-from-dispatch) интеграция: отправьте задачу со своего телефона, получите сеанс здесь
* [Запланированные задачи](/ru/desktop-scheduled-tasks), которые запускают Claude по расписанию
...
<Note>
  Макет рабочей области, терминал, редактор файлов, боковые чаты и режимы просмотра, описанные на этой странице, требуют Claude Desktop v1.2581.0 или позже. Откройте **Claude → Check for Updates** на macOS или **Help → Check for Updates** на Windows для обновления.
</Note>

На этой странице рассматриваются [работа с кодом](#work-with-code), [организация рабочей области](#arrange-your-workspace), [использование компьютера](#let-claude-use-your-computer), [управление сеансами](#manage-sessions), [расширение Claude Code](#extend-claude-code) и [конфигурация](#environment-configuration). Она также включает [сравнение CLI](#coming-from-the-cli) и [устранение неполадок](#troubleshooting).
...
| **Auto**               | `auto`              | Claude выполняет все действия с фоновыми проверками безопасности, которые проверяют соответствие вашему запросу. Снижает подсказки разрешений при сохранении надзора. В настоящее время исследовательский предпросмотр. Доступно на планах Max, Team, Enterprise и API. Требует Claude Sonnet 4.6, Opus 4.6 или Opus 4.7 на планах Team, Enterprise и API; только Claude Opus 4.7 на планах Max. Недоступно на планах Pro или у сторонних провайдеров. Включите в Settings → Claude Code. |
...
Чтобы включить gitignored файлы, такие как `.env`, в новые worktrees, создайте [`.worktreeinclude` файл](/ru/common-workflows#copy-gitignored-files-to-worktrees) в корне вашего проекта.
...
Удалённая машина должна работать на Linux или macOS, и Claude Code должен быть установлен на ней. После подключения SSH-сеансы поддерживают режимы разрешений, коннекторы, плагины и MCP servers.
...
| `autoMode`                                 | настройте, что классификатор auto mode доверяет и блокирует во всей вашей организации. См. [Configure the auto mode classifier](/ru/permissions#configure-the-auto-mode-classifier).                                   |
...
* **Сторонние провайдеры**: Desktop подключается к API Anthropic напрямую. Развёртывания предприятия могут конфигурировать Vertex AI и провайдеров шлюза через [управляемые параметры](https://support.claude.com/en/articles/12622667-enterprise-configuration). Для Bedrock или Foundry используйте [CLI](/ru/quickstart).
