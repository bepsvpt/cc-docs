...
> Получите больше возможностей от Claude Code Desktop: использование компьютера, отправка сеансов из приложения на телефоне, параллельные сеансы с изоляцией Git, визуальный просмотр различий, предпросмотр приложений, мониторинг PR, коннекторы и конфигурация для предприятий.
...
* [Использование компьютера](#let-claude-use-your-computer) для открытия приложений и управления экраном на macOS
...
Использование компьютера отключено по умолчанию. [Включите его в Settings](#enable-computer-use) и предоставьте необходимые разрешения macOS, прежде чем Claude сможет управлять вашим экраном.
...
<Steps>
  ...
  <Step title="Включение переключателя">
    В приложении desktop перейдите в **Settings > General** (под **Desktop app**). Найдите переключатель **Computer use** и включите его.

    Если вы не видите переключатель, подтвердите, что вы на macOS с планом Pro или Max, затем обновите и перезагрузите приложение.
  </Step>

  <Step title="Предоставление разрешений macOS">
    Перед тем как переключатель вступит в силу, предоставьте два разрешения системы macOS:
...
Оба вида появляются в одной сетке задач. Нажмите **New task**, чтобы выбрать, какой вид создать. Остальная часть этого раздела охватывает локальные задачи; для удалённых задач см. [Cloud scheduled tasks](/ru/web-scheduled-tasks).

См. [Как работают запланированные задачи](#how-scheduled-tasks-run) для получения подробной информации о пропущенных запусках и поведении наверстывания для локальных задач.

<Note>
  По умолчанию локальные запланированные задачи работают против любого состояния вашего рабочего каталога, включая незафиксированные изменения. Включите переключатель worktree в ввод подсказки, чтобы дать каждому запуску свой собственный изолированный Git worktree, так же как [параллельные сеансы](#work-in-parallel-with-sessions).
</Note>
...
| Использование компьютера                             | [Включение через `/mcp`](/ru/computer-use) на macOS       | [Управление приложениями и экраном](#let-claude-use-your-computer) на macOS                   |
