...
WSL1 не поддерживает sandboxing, так как ему не хватает необходимых примитивов пространства имен Linux. Если вы видите `Sandboxing requires WSL2`, обновите вашу дистрибьюцию на WSL2 или запустите Claude Code без sandboxing.
...
По умолчанию, если sandbox не может запуститься (отсутствуют зависимости или неподдерживаемая платформа), Claude Code показывает предупреждение и выполняет команды без sandboxing. Чтобы сделать это жестким отказом вместо этого, установите [`sandbox.failIfUnavailable`](/ru/settings#sandbox-settings) в `true`. Это предназначено для управляемых развертываний, которые требуют sandboxing в качестве шлюза безопасности.
...
* Используйте `deniedDomains` sandbox для блокирования определенных доменов даже когда более широкий подстановочный знак `allowedDomains` иначе разрешил бы их
...
* **Контейнерами разработки**: Используйте с [dev containers](/ru/devcontainer) для дополнительной изоляции
