...
| `true`           | Все инструменты MCP откладываются. Claude Code отправляет заголовок бета-версии даже на Vertex AI и через прокси. Запросы завершаются ошибкой, если backend не поддерживает блоки `tool_reference` |
