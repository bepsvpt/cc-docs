...
Запуск [`/init`](/ru/commands) в репозитории, который уже имеет `AGENTS.md`, читает его и включает соответствующие части в созданный `CLAUDE.md`. Он также читает другие конфигурации инструментов, такие как `.cursorrules` и `.windsurfrules`.
...
Чтобы сохранить auto memory в другом местоположении, установите `autoMemoryDirectory` в ваших пользовательских настройках в `~/.claude/settings.json`:
