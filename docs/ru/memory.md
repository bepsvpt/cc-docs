...
> Дайте Claude постоянные инструкции с помощью файлов CLAUDE.md и позвольте Claude автоматически накапливать знания с помощью автоматической памяти.
...
* **Автоматическая память**: заметки, которые Claude пишет сам на основе ваших исправлений и предпочтений

На этой странице рассматривается, как:

* [Писать и организовывать файлы CLAUDE.md](#claudemd-files)
* [Ограничивать правила определёнными типами файлов](#organize-rules-with-clauderules) с помощью `.claude/rules/`
* [Настраивать автоматическую память](#auto-memory) так, чтобы Claude автоматически вел заметки
* [Устранять неполадки](#troubleshoot-memory-issues) когда инструкции не соблюдаются

## CLAUDE.md и автоматическая память
...
|                      | Файлы CLAUDE.md                                              | Автоматическая память                                                   |
| :------------------- | :----------------------------------------------------------- | :---------------------------------------------------------------------- |
...
| **Используется для** | Стандарты кодирования, рабочие процессы, архитектура проекта | Команды сборки, идеи отладки, предпочтения, которые обнаруживает Claude |

Используйте файлы CLAUDE.md, когда вы хотите направить поведение Claude. Автоматическая память позволяет Claude учиться на ваших исправлениях без ручных усилий.

Subagents также могут поддерживать свою собственную автоматическую память. Подробнее см. в разделе [конфигурация subagent](/ru/sub-agents#enable-persistent-memory).
...
Файлы CLAUDE.md в иерархии каталогов выше рабочего каталога загружаются полностью при запуске. Файлы CLAUDE.md в подкаталогах загружаются по требованию, когда Claude читает файлы в этих каталогах. Полный порядок разрешения см. в разделе [Как загружаются файлы CLAUDE.md](#how-claudemd-files-load).

Для больших проектов вы можете разбить инструкции на файлы, специфичные для темы, используя [правила проекта](#organize-rules-with-clauderules). Правила позволяют ограничить инструкции определёнными типами файлов или подкаталогами.
...
**Размер**: целевой показатель менее 200 строк на файл CLAUDE.md. Более длинные файлы потребляют больше контекста и снижают соблюдение. Если ваши инструкции становятся большими, разделите их с помощью [импорта](#import-additional-files) или файлов [`.claude/rules/`](#organize-rules-with-clauderules).
...
**Согласованность**: если два правила противоречат друг другу, Claude может выбрать одно произвольно. Периодически проверяйте ваши файлы CLAUDE.md, вложенные файлы CLAUDE.md в подкаталогах и файлы [`.claude/rules/`](#organize-rules-with-clauderules), чтобы удалить устаревшие или конфликтующие инструкции. В монорепозиториях используйте [`claudeMdExcludes`](#exclude-specific-claudemd-files) для пропуска файлов CLAUDE.md от других команд, которые не имеют отношения к вашей работе.
...
Чтобы включить README, package.json и руководство по рабочему процессу, ссылайтесь на них с помощью синтаксиса `@` в любом месте вашего CLAUDE.md:
...
Для более структурированного подхода к организации инструкций см. [`.claude/rules/`](#organize-rules-with-clauderules).
...
Если вы работаете в большом монорепозитории, где подбираются файлы CLAUDE.md других команд, используйте [`claudeMdExcludes`](#exclude-specific-claudemd-files) для их пропуска.
...
#### Делитесь правилами между проектами с помощью символических ссылок

Каталог `.claude/rules/` поддерживает символические ссылки, поэтому вы можете поддерживать общий набор правил и связывать их с несколькими проектами. Символические ссылки разрешаются и загружаются нормально, а циклические символические ссылки обнаруживаются и обрабатываются корректно.
...
Автоматическая память позволяет Claude накапливать знания между сеансами без вашего участия. Claude сохраняет заметки для себя по мере работы: команды сборки, идеи отладки, заметки об архитектуре, предпочтения стиля кода и привычки рабочего процесса. Claude не сохраняет что-то каждый сеанс. Он решает, что стоит помнить, на основе того, будет ли информация полезна в будущем разговоре.

<Note>
  Автоматическая память требует Claude Code v2.1.59 или позже. Проверьте вашу версию с помощью `claude --version`.
</Note>

### Включите или отключите автоматическую память

Автоматическая память включена по умолчанию. Чтобы переключить её, откройте `/memory` в сеансе и используйте переключатель автоматической памяти, или установите `autoMemoryEnabled` в настройках вашего проекта:
...
Чтобы отключить автоматическую память через переменную окружения, установите `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`.

### Местоположение хранилища

Каждый проект получает свой собственный каталог памяти в `~/.claude/projects/<project>/memory/`. Путь `<project>` получается из репозитория git, поэтому все worktrees и подкаталоги в одном репозитории используют один каталог автоматической памяти. Вне репозитория git вместо этого используется корень проекта.

Чтобы сохранить автоматическую память в другом местоположении, установите `autoMemoryDirectory` в ваших пользовательских или локальных настройках:
...
Этот параметр принимается из политики, локальных и пользовательских настроек. Он не принимается из настроек проекта (`.claude/settings.json`), чтобы предотвратить перенаправление общего проекта записей автоматической памяти в чувствительные местоположения.
...
Автоматическая память зависит от машины. Все worktrees и подкаталоги в одном репозитории git используют один каталог автоматической памяти. Файлы не общие между машинами или облачными средами.

### Как это работает

Первые 200 строк `MEMORY.md` загружаются в начале каждого разговора. Содержимое после строки 200 не загружается при запуске сеанса. Claude держит `MEMORY.md` лаконичным, перемещая подробные заметки в отдельные файлы по темам.
...
Файлы автоматической памяти — это простой markdown, который вы можете редактировать или удалять в любое время. Запустите [`/memory`](#view-and-edit-with-memory) для просмотра и открытия файлов памяти из сеанса.

## Просмотр и редактирование с помощью `/memory`

Команда `/memory` перечисляет все файлы CLAUDE.md и rules, загруженные в вашем текущем сеансе, позволяет переключать автоматическую память включена или выключена, и предоставляет ссылку для открытия папки автоматической памяти. Выберите любой файл для открытия его в вашем редакторе.

Когда вы просите Claude что-то запомнить, например "всегда используйте pnpm, а не npm" или "помните, что тесты API требуют локального экземпляра Redis", Claude сохраняет это в автоматическую память. Чтобы добавить инструкции в CLAUDE.md, попросите Claude напрямую, например "добавьте это в CLAUDE.md", или отредактируйте файл самостоятельно через `/memory`.

## Устранение неполадок с памятью

Это наиболее распространённые проблемы с CLAUDE.md и автоматической памятью, а также шаги для их отладки.

### Claude не следует моему CLAUDE.md

CLAUDE.md — это контекст, а не принуждение. Claude читает его и пытается следовать ему, но нет гарантии строгого соответствия, особенно для расплывчатых или конфликтующих инструкций.

Для отладки:

* Запустите `/memory` для проверки загрузки ваших файлов CLAUDE.md. Если файл не указан, Claude не может его видеть.
* Проверьте, что соответствующий CLAUDE.md находится в местоположении, которое загружается для вашего сеанса (см. [Выберите, где разместить файлы CLAUDE.md](#choose-where-to-put-claudemd-files)).
...
<Tip>
  Используйте [hook `InstructionsLoaded`](/ru/hooks#instructionsloaded) для логирования точно, какие файлы инструкций загружены, когда они загружаются и почему. Это полезно для отладки правил, специфичных для пути, или ленивых загруженных файлов в подкаталогах.
</Tip>

### Я не знаю, что сохранила автоматическая память

Запустите `/memory` и выберите папку автоматической памяти для просмотра того, что Claude сохранил. Всё это простой markdown, который вы можете читать, редактировать или удалять.

### Мой CLAUDE.md слишком большой

Файлы более 200 строк потребляют больше контекста и могут снизить соблюдение. Переместите подробное содержимое в отдельные файлы, на которые ссылаются импорты `@path` (см. [Импортируйте дополнительные файлы](#import-additional-files)), или разделите ваши инструкции между файлами [`.claude/rules/`](#organize-rules-with-clauderules).
...
* [Subagent memory](/ru/sub-agents#enable-persistent-memory): позвольте subagents поддерживать свою собственную автоматическую память
