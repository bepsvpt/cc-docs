...
Все обнаруженные файлы объединяются в контекст, а не переопределяют друг друга. В каждом каталоге `CLAUDE.local.md` добавляется после `CLAUDE.md`, поэтому когда инструкции конфликтуют, ваши личные заметки — это последнее, что Claude читает на этом уровне.
...
CLAUDE.md полностью выживает при сжатии: после `/compact` Claude повторно читает его с диска и повторно вводит его в сеанс. Вложенные файлы CLAUDE.md в подкаталогах не повторно вводятся автоматически; они перезагружаются в следующий раз, когда Claude читает файл в этом подкаталоге.
