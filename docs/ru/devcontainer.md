...
На этой странице рассматривается [установка Claude Code в контейнер разработки](#add-claude-code-to-your-dev-container) и следующие темы конфигурации. Каждая тема является самостоятельной, поэтому переходите к тем, которые соответствуют тому, что вам нужно настроить:

* [Сохранение аутентификации и параметров при перестроении](#persist-authentication-and-settings-across-rebuilds)
* [Применение политики организации](#enforce-organization-policy)
* [Ограничение исходящего сетевого трафика](#restrict-network-egress)
* [Запуск без запросов разрешений](#run-without-permission-prompts)
...
* [Режимы разрешений](/ru/permission-modes): полный диапазон от режима плана к автоматическому режиму к обходу и когда использовать каждый
