...
<Steps>
  ...
  <Step title="Создайте манифест plugin">
    ...
    | `version`     | Опционально. Если установлено, пользователи получают обновления только когда вы увеличиваете это поле. Если опущено и ваш plugin распространяется через git, используется SHA коммита и каждый коммит считается новой версией. См. [управление версиями](/ru/plugins-reference#version-management). |
...
2. **Выберите стратегию версионирования**: Решите, устанавливать ли явную `version` или полагаться на SHA коммита git. См. [управление версиями](/ru/plugins-reference#version-management)
