...
* Claude Code версии 1.0.33 или позже (запустите `claude --version` для проверки)
...
<Steps>
  ...
  <Step title="Протестируйте ваш plugin">
    ...
    <Note>
      **Почему пространство имён?** Skills plugin всегда имеют пространство имён (например, `/greet:hello`) для предотвращения конфликтов, когда несколько plugins имеют skills с одинаковым именем.
...
По мере внесения изменений в ваш plugin запустите `/reload-plugins` для применения обновлений без перезагрузки. Это перезагружает commands, skills, agents, hooks, plugin MCP servers и plugin LSP servers. Протестируйте компоненты вашего plugin:
