...
По мере внесения изменений в ваш plugin запустите `/reload-plugins` для применения обновлений без перезагрузки. Изменения конфигурации LSP server по-прежнему требуют полной перезагрузки. Протестируйте компоненты вашего plugin:
