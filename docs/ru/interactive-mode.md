...
| `Ctrl+L`                                            | Очистить ввод запроса и перерисовать экран                                          | Очищает введенный текст и принудительно выполняет полную перерисовку терминала. История разговора сохраняется. Используйте это для восстановления, если дисплей становится искаженным или частично пустым                                                                                                                                             |
...
| Клавиша Option                 | `Option+Enter`    | После включения [Option как Meta](/ru/terminal-config#enable-option-key-shortcuts-on-macos) на macOS |
| Shift+Enter                    | `Shift+Enter`     | Встроено в iTerm2, WezTerm, Ghostty, Kitty, Warp, Apple Terminal                                     |
...
<Tip>
  Shift+Enter работает без конфигурации в iTerm2, WezTerm, Ghostty, Kitty, Warp и Apple Terminal. Для VS Code, Cursor, Windsurf, Alacritty и Zed запустите `/terminal-setup` для установки привязки.
</Tip>
...
| :--------------- | :---------------------- | :---------------------------------------------------------------- |
| `/` в начале     | Команда или skill       | См. [команды](#commands) и [skills](/ru/skills)                   |
...
| `[`                  | Записать полный разговор в собственный буфер прокрутки вашего терминала, чтобы `Cmd+F`, режим копирования tmux и другие встроенные инструменты могли его искать. Требует [полноэкранного отображения](/ru/fullscreen#search-and-review-the-conversation) |
| `v`                  | Записать разговор во временный файл и открыть его в `$VISUAL` или `$EDITOR`. Требует [полноэкранного отображения](/ru/fullscreen)                                                                                                                        |
...
Полный список команд, включенных в Claude Code, см. в [справочнике команд](/ru/commands).
...
Резюме сеанса включено по умолчанию для каждого плана и провайдера. Чтобы переопределить переключатель `/config`, установите [`CLAUDE_CODE_ENABLE_AWAY_SUMMARY`](/ru/env-vars) на `0` или `1`. Резюме всегда пропускается в неинтерактивном режиме.
