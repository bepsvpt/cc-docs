...
| `Option+T` (macOS) или `Alt+T` (Windows/Linux)      | Переключить расширенное мышление                                                    | Включите или отключите режим расширенного мышления. Сначала запустите `/terminal-setup` для включения этого сочетания клавиш                                                        |
...
| `q`, `Ctrl+C`, `Esc` | Выход из просмотра транскрипции. `Ctrl+C` и `Esc` можно переназначить через [`transcript:exit`](/ru/keybindings); `q` не переназначаемо |
