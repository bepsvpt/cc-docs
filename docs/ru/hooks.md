...
| Поле     | Описание                                                                                     |
| :------- | :------------------------------------------------------------------------------------------- |
| `ok`     | `true` разрешает действие, `false` предотвращает его. См. поведение для каждого события ниже |
| `reason` | Требуется при `ok` равном `false`. Объяснение для решения                                    |
...
* `PostToolUse`, `PostToolBatch`, `UserPromptSubmit` и `UserPromptExpansion`: ход заканчивается и причина появляется в чате как строка предупреждения, эквивалентно возврату `"continue": false` из command hook
