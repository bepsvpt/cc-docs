...
<div style={{maxWidth: "500px", margin: "0 auto"}}>
  <Frame>
    <img src="https://mintcdn.com/claude-code/2YzYcIR7V1VggfgF/images/hooks-lifecycle.svg?fit=max&auto=format&n=2YzYcIR7V1VggfgF&q=85&s=3004e6c5dc95c4fe7fa3eb40fdc4176c" alt="Hook 生命周期图，显示从 SessionStart 通过代理循环到 SessionEnd 的 hooks 序列，WorktreeCreate、WorktreeRemove 和 InstructionsLoaded 作为独立异步事件" width="520" height="1100" data-path="images/hooks-lifecycle.svg" />
  </Frame>
</div>
...
要了解这些部分如何组合在一起，请考虑这个 `PreToolUse` hook，它阻止破坏性 shell 命令。该 hook 在每次 Bash 工具调用之前运行 `block-rm.sh`：
...
<Frame>
  <img src="https://mintcdn.com/claude-code/c5r9_6tjPMzFdDDT/images/hook-resolution.svg?fit=max&auto=format&n=c5r9_6tjPMzFdDDT&q=85&s=ad667ee6d86ab2276aa48a4e73e220df" alt="Hook 解析流：PreToolUse 事件触发，匹配器检查 Bash 匹配，hook 处理程序运行，结果返回到 Claude Code" width="780" height="290" data-path="images/hook-resolution.svg" />
</Frame>
...
2. 添加 [匹配器组](#matcher-patterns) 来过滤何时触发，如"仅针对 Bash 工具"
...
| [Skill](/zh-CN/skills) 或 [agent](/zh-CN/sub-agents) frontmatter | 组件活跃时  | 是，在组件文件中定义   |

有关设置文件解析的详细信息，请参阅 [settings](/zh-CN/settings)。企业管理员可以使用 `allowManagedHooksOnly` 来阻止用户、项目和插件 hooks。请参阅 [Hook 配置](/zh-CN/settings#hook-configuration)。
...
匹配器是一个正则表达式，所以 `Edit|Write` 匹配任一工具，`Notebook.*` 匹配任何以 Notebook 开头的工具。匹配器针对 Claude Code 在 stdin 上发送给您的 hook 的 [JSON 输入](#hook-input-and-output) 中的字段运行。对于工具事件，该字段是 `tool_name`。每个 [hook 事件](#hook-events) 部分列出了该事件的完整匹配器值集和输入架构。
...
`UserPromptSubmit`、`Stop`、`TeammateIdle`、`TaskCompleted`、`WorktreeCreate`、`WorktreeRemove` 和 `InstructionsLoaded` 不支持匹配器，总是在每次出现时触发。如果您向这些事件添加 `matcher` 字段，它会被静默忽略。

#### 匹配 MCP 工具

[MCP](/zh-CN/mcp) 服务器工具在工具事件中显示为常规工具（`PreToolUse`、`PostToolUse`、`PostToolUseFailure`、`PermissionRequest`），因此您可以像匹配任何其他工具名称一样匹配它们。
...
* **[提示 hooks](#prompt-and-agent-hook-fields)**（`type: "prompt"`）：向 Claude 模型发送提示进行单轮评估。模型返回 yes/no 决定作为 JSON。请参阅 [基于提示的 hooks](#prompt-based-hooks)。
...
<Note>
  HTTP hooks 必须通过直接编辑设置 JSON 来配置。`/hooks` 交互菜单仅支持添加命令 hooks。
</Note>
...
所有匹配的 hooks 并行运行，相同的处理程序会自动去重。命令 hooks 按命令字符串去重，HTTP hooks 按 URL 去重。处理程序在当前目录中运行，使用 Claude Code 的环境。`$CLAUDE_CODE_REMOTE` 环境变量在远程 web 环境中设置为 `"true"`，在本地 CLI 中未设置。

### 按路径引用脚本

使用环境变量来引用相对于项目或插件根目录的 hook 脚本，无论 hook 运行时的工作目录如何：

* `$CLAUDE_PROJECT_DIR`：项目根目录。用引号包装以处理包含空格的路径。
* `${CLAUDE_PLUGIN_ROOT}`：插件的根目录，用于与 [plugin](/zh-CN/plugins) 捆绑的脚本。
...
在 Claude Code 中输入 `/hooks` 以打开交互式 hooks 管理器，您可以在其中查看、添加和删除 hooks，而无需直接编辑设置文件。有关分步演练，请参阅指南中的 [设置您的第一个 hook](/zh-CN/hooks-guide#set-up-your-first-hook)。

菜单中的每个 hook 都标有括号前缀，指示其来源：
...
要移除 hook，请从设置 JSON 文件中删除其条目，或使用 `/hooks` 菜单并选择 hook 来删除它。

要临时禁用所有 hooks 而不移除它们，请在设置文件中设置 `"disableAllHooks": true` 或使用 `/hooks` 菜单中的切换。没有办法在保持 hook 在配置中的同时禁用单个 hook。

`disableAllHooks` 设置遵循托管设置层次结构。如果管理员通过托管策略设置配置了 hooks，则在用户、项目或本地设置中设置的 `disableAllHooks` 无法禁用这些托管 hooks。仅在托管设置级别设置的 `disableAllHooks` 可以禁用托管 hooks。

对设置文件中 hooks 的直接编辑不会立即生效。Claude Code 在启动时捕获 hooks 的快照，并在整个会话中使用它。这可以防止恶意或意外的 hook 修改在会话中途生效，而无需您的审查。如果 hooks 在外部被修改，Claude Code 会警告您，并要求在 `/hooks` 菜单中进行审查，然后更改才能应用。
...
| `permission_mode` | 当前 [权限模式](/zh-CN/permissions#permission-modes)：`"default"`、`"plan"`、`"acceptEdits"`、`"dontAsk"` 或 `"bypassPermissions"` |
...
来自您的 hook 命令的退出代码告诉 Claude Code 该操作是应该继续、被阻止还是被忽略。
...
**退出 2** 表示阻止错误。Claude Code 忽略 stdout 和其中的任何 JSON。相反，stderr 文本被反馈给 Claude 作为错误消息。效果取决于事件：`PreToolUse` 阻止工具调用，`UserPromptSubmit` 拒绝提示，等等。有关完整列表，请参阅 [退出代码 2 行为](#exit-code-2-behavior-per-event)。
...
退出代码让您允许或阻止，但 JSON 输出提供更细粒度的控制。与其使用代码 2 退出来阻止，不如退出 0 并将 JSON 对象打印到 stdout。Claude Code 从该 JSON 读取特定字段以控制行为，包括 [决定控制](#decision-control) 来阻止、允许或升级给用户。

<Note>
  您必须为每个 hook 选择一种方法，而不是两种：要么单独使用退出代码进行信号，要么退出 0 并打印 JSON 以进行结构化控制。Claude Code 仅在退出 0 时处理 JSON。如果您退出 2，任何 JSON 都会被忽略。
</Note>
...
| `stopReason`     | 无       | hook 运行后向用户显示的消息，当 `continue` 为 `false` 时。不向 Claude 显示 |
...
| TeammateIdle、TaskCompleted                                                     | 退出代码或 `continue: false` | 退出代码 2 用 stderr 反馈阻止操作。JSON `{"continue": false, "stopReason": "..."}` 也停止队友，匹配 `Stop` hook 行为 |
| PreToolUse                                                                     | `hookSpecificOutput`    | `permissionDecision`（allow/deny/ask）、`permissionDecisionReason`                                |
| PermissionRequest                                                              | `hookSpecificOutput`    | `decision.behavior`（allow/deny）                                                                |
| WorktreeCreate                                                                 | stdout 路径               | Hook 打印创建的 worktree 的绝对路径。非零退出失败创建                                                             |
| WorktreeRemove、Notification、SessionEnd、PreCompact、InstructionsLoaded           | 无                       | 无决定控制。用于日志记录或清理等副作用                                                                            |
...
有关扩展示例，包括 Bash 命令验证、提示过滤和自动批准脚本，请参阅指南中的 [您可以自动化的内容](/zh-CN/hooks-guide#what-you-can-automate) 和 [Bash 命令验证器参考实现](https://github.com/anthropics/claude-code/blob/main/examples/hooks/bash_command_validator_example.py)。

## Hook 事件

每个事件对应于 Claude Code 生命周期中 hooks 可以运行的一个点。下面的部分按照生命周期的顺序排列：从会话设置通过代理循环到会话结束。每个部分描述事件何时触发、它支持什么匹配器、它接收的 JSON 输入以及如何通过输出控制行为。

### SessionStart

在 Claude Code 启动新会话或恢复现有会话时运行。对于加载开发上下文（如现有问题或代码库的最近更改）或设置环境变量很有用。对于不需要脚本的静态上下文，请改用 [CLAUDE.md](/zh-CN/memory)。

SessionStart 在每个会话上运行，所以保持这些 hooks 快速。仅支持 `type: "command"` hooks。
...
当 `CLAUDE.md` 或 `.claude/rules/*.md` 文件加载到上下文中时触发。此事件在会话启动时为急切加载的文件触发，稍后当文件被懒加载时再次触发，例如当 Claude 访问包含嵌套 `CLAUDE.md` 的子目录或条件规则与 `paths:` frontmatter 匹配时。hook 不支持阻止或决定控制。它异步运行以用于可观测性目的。

InstructionsLoaded 不支持匹配器，在每次加载时触发。
...
| `globs`             | 文件的 `paths:` frontmatter 中的路径 glob 模式（如果有）。仅对 `path_glob_match` 加载存在              |
...
在 Claude 创建工具参数后和处理工具调用之前运行。在工具名称上匹配：`Bash`、`Edit`、`Write`、`Read`、`Glob`、`Grep`、`Agent`、`WebFetch`、`WebSearch` 和任何 [MCP 工具名称](#match-mcp-tools)。

使用 [PreToolUse 决定控制](#pretooluse-decision-control) 来允许、拒绝或要求使用工具的权限。
...
| `glob`        | string  | `"*.ts"`         | 可选的 glob 模式来过滤文件                                                          |
...
| `model`         | string | `"sonnet"`                 | 可选的模型别名来覆盖默认值 |
...
<Note>
  PreToolUse 之前使用顶级 `decision` 和 `reason` 字段，但这些对此事件已弃用。改用 `hookSpecificOutput.permissionDecision` 和 `hookSpecificOutput.permissionDecisionReason`。弃用的值 `"approve"` 和 `"block"` 分别映射到 `"allow"` 和 `"deny"`。PostToolUse 和 Stop 等其他事件继续使用顶级 `decision` 和 `reason` 作为其当前格式。
</Note>
...
| `updatedPermissions` | 仅对 `"allow"`：应用权限规则更新，等同于用户选择"总是允许"选项 |
...
除了 [通用输入字段](#common-input-fields) 外，Notification hooks 还接收包含通知文本的 `message`、可选的 `title` 和 `notification_type` 指示哪个类型触发。
...
Notification hooks 无法阻止或修改通知。除了所有 hooks 可用的 [JSON 输出字段](#json-output) 外，您可以返回 `additionalContext` 来向对话添加上下文：
...
当通过 Agent 工具生成 Claude Code subagent 时运行。支持匹配器来按代理类型名称过滤（内置代理如 `Bash`、`Explore`、`Plan` 或来自 `.claude/agents/` 的自定义代理名称）。

#### SubagentStart 输入

除了 [通用输入字段](#common-input-fields) 外，SubagentStart hooks 还接收 `agent_id` 与 subagent 的唯一标识符和 `agent_type` 与代理名称（内置代理如 `"Bash"`、`"Explore"`、`"Plan"` 或自定义代理名称）。
...
在 Claude Code subagent 完成响应时运行。在代理类型上匹配，与 SubagentStart 相同的值。
...
在 [agent team](/zh-CN/agent-teams) 队友在完成其轮次后即将空闲时运行。使用此来在队友停止工作之前强制执行质量门，例如要求通过 lint 检查或验证输出文件存在。
...
当任务被标记为已完成时运行。这在两种情况下触发：当任何代理通过 TaskUpdate 工具显式标记任务为已完成时，或当 [agent team](/zh-CN/agent-teams) 队友完成其轮次且有进行中的任务时。使用此来强制执行完成标准，如通过测试或 lint 检查，然后任务才能关闭。
...
在会话期间配置文件更改时运行。使用此来审计设置更改、强制执行安全策略或阻止对配置文件的未授权修改。
...
`policy_settings` 更改无法被阻止。Hooks 仍然对 `policy_settings` 源触发，因此您可以使用它们进行审计日志记录，但任何阻止决定都被忽略。这确保企业管理的设置总是生效。
...
hook 必须在 stdout 上打印创建的 worktree 目录的绝对路径。Claude Code 使用此路径作为隔离会话的工作目录。
...
hook 必须在 stdout 上打印创建的 worktree 目录的绝对路径。如果 hook 失败或不产生输出，worktree 创建失败并出现错误。

WorktreeCreate hooks 不使用标准的允许/阻止决定模型。相反，hook 的成功或失败决定结果。仅支持 `type: "command"` hooks。
...
Claude Code 将 WorktreeCreate 在 stdout 上打印的路径作为 `worktree_path` 在 hook 输入中传递。此示例读取该路径并移除目录：
...
在 Claude Code 会话结束时运行。对于清理任务、记录会话统计或保存会话状态很有用。支持匹配器来按退出原因过滤。

hook 输入中的 `reason` 字段指示会话为何结束：

| 原因                            | 描述                |
| :---------------------------- | :---------------- |
| `clear`                       | 会话用 `/clear` 命令清除 |
| `logout`                      | 用户登出              |
...
此 `Stop` hook 使用详细提示在允许 Claude 停止之前检查三个条件。如果 `"ok"` 为 `false`，Claude 继续工作，提供的原因作为其下一条指令。`SubagentStop` hooks 使用相同的格式来评估 [subagent](/zh-CN/sub-agents) 是否应该停止：
...
基于代理的 hooks（`type: "agent"`）类似于基于提示的 hooks，但具有多轮工具访问。代理 hook 不是单个 LLM 调用，而是生成一个可以读取文件、搜索代码和检查代码库来验证条件的 subagent。代理 hooks 支持与基于提示的 hooks 相同的事件。
...
2. subagent 可以使用 Read、Grep 和 Glob 等工具来调查
...
将 `type` 设置为 `"agent"` 并提供 `prompt` 字符串。配置字段与 [提示 hooks](#prompt-hook-configuration) 相同，超时更长：
...
此 `Stop` hook 在允许 Claude 完成之前验证所有单元测试通过：
...
向命令 hook 的配置添加 `"async": true` 以在后台运行它而不阻止 Claude。此字段仅在 `type: "command"` hooks 上可用。
...
当异步 hook 触发时，Claude Code 启动 hook 进程并立即继续，不等待它完成。hook 在 stdin 上接收与同步 hook 相同的 JSON 输入。
...
* 异步 hooks 无法阻止工具调用或返回决定。当 hook 完成时，触发操作已经进行。
* Hook 输出在下一个对话轮次上传递。如果会话空闲，响应等待直到下一个用户交互。
* 每次执行创建一个单独的后台进程。同一异步 hook 的多次触发之间没有去重。
...
<Warning>
  命令 hooks 以您的完整用户权限执行 shell 命令。它们可以修改、删除或访问您的用户帐户可以访问的任何文件。在将任何 hook 命令添加到您的配置之前，请审查并测试它们。
</Warning>
