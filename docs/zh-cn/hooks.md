...
<Frame>
  <img src="https://mintcdn.com/claude-code/c5r9_6tjPMzFdDDT/images/hook-resolution.svg?fit=max&auto=format&n=c5r9_6tjPMzFdDDT&q=85&s=ad667ee6d86ab2276aa48a4e73e220df" alt="Hook 解析流程：PreToolUse 事件触发，匹配器检查 Bash 匹配，hook 处理程序运行，结果返回到 Claude Code" width="780" height="290" data-path="images/hook-resolution.svg" />
</Frame>
...
* `${CLAUDE_PLUGIN_ROOT}`：插件的根目录，用于与[插件](/zh-CN/plugins)捆绑的脚本。
...
| WorktreeRemove、Notification、SessionEnd、PreCompact、PostCompact、InstructionsLoaded | 无                       | 无决定控制。用于日志记录或清理等副作用                                                                                |
...
当[代理团队](/zh-CN/agent-teams)队友在完成其轮次后即将空闲时运行。使用此来在队友停止工作之前强制执行质量门，例如要求通过 lint 检查或验证输出文件存在。
...
SessionEnd hooks 的默认超时为 1.5 秒。这适用于会话退出和 `/clear`。如果您的 hooks 需要更多时间，请将 `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` 环境变量设置为毫秒的更高值。任何每个 hook 的 `timeout` 设置也受此值的限制。
