...
| `includeGitInstructions`          | 在 Claude 的系统提示中包含内置提交和 PR 工作流说明（默认：`true`）。设置为 `false` 以删除这些说明，例如在使用您自己的 git 工作流 skills 时。当设置 `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` 环境变量时，它优先于此设置                            | `false`                                                                 |
...
| `./` 或无前缀 | 相对路径（由 sandbox 运行时解析） | `./output`                        |
