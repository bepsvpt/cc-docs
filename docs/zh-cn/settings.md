...
| `cleanupPeriodDays`               | 非活跃时间超过此期间的会话在启动时被删除（默认：30 天）。<br /><br />设置为 `0` 会在启动时删除所有现有记录，并完全禁用会话持久性。不会写入新的 `.jsonl` 文件，`/resume` 不显示任何对话，hooks 接收空的 `transcript_path`。                                                                     | `20`                                                                    |
...
| `autoMode`                        | 自定义[自动模式](/zh-CN/permission-modes#eliminate-prompts-with-auto-mode)分类器阻止和允许的内容。包含 `environment`、`allow` 和 `soft_deny` 散文规则数组。请参阅[配置自动模式分类器](/zh-CN/permissions#configure-the-auto-mode-classifier)。不从共享项目设置读取     | `{"environment": ["Trusted repo: github.example.com/acme"]}`            |
...
| `effortLevel`                     | 跨会话持久化[努力级别](/zh-CN/model-config#adjust-effort-level)。接受 `"low"`、`"medium"` 或 `"high"`。当您运行 `/effort low`、`/effort medium` 或 `/effort high` 时自动写入。在 Opus 4.6 和 Sonnet 4.6 上支持                                     | `"medium"`                                                              |
...
| `teammateMode`                    | [agent team](/zh-CN/agent-teams) 队友的显示方式：`auto`（在 tmux 或 iTerm2 中选择分割窗格，否则进程内）、`in-process` 或 `tmux`。请参阅[设置 agent teams](/zh-CN/agent-teams#set-up-agent-teams)                                                   | `"in-process"`                                                          |
...
| `editorMode`                 | 输入提示的快捷键模式：`"normal"` 或 `"vim"`。默认：`"normal"`。当您运行 `/vim` 时自动写入。在 `/config` 中显示为**快捷键模式**                                                                                       | `"vim"` |
...
| `defaultMode`                  | 打开 Claude Code 时的默认[权限模式](/zh-CN/permission-modes)                                                                                                 | `"acceptEdits"`                                                        |
...
| `filesystem.allowManagedReadPathsOnly` | （仅 Managed 设置）仅尊重来自 managed 设置的 `allowRead` 路径。来自用户、项目和本地设置的 `allowRead` 条目被忽略。默认：false                                                                                                    | `true`                          |
| `network.allowUnixSockets`             | sandbox 中可访问的 Unix socket 路径（用于 SSH agents 等）                                                                                                                                              | `["~/.ssh/agent-socket"]`       |
...
**仅 Managed 设置**：控制用户允许添加哪些插件市场。此设置只能在 [managed 设置](/zh-CN/settings#settings-files)中配置，为管理员提供对市场源的严格控制。
