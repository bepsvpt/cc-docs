...
<Steps>
  ...
  <Step title="为每个存储库设置审查触发器">
    ...
    * **手动**：仅当有人[在 PR 上注释 `@claude review`](#manually-trigger-reviews) 时才启动审查；之后对该 PR 的后续推送会自动审查
...
在任何模式下，注释 `@claude review` [选择 PR 进入推送触发审查](#manually-trigger-reviews)，因此在该注释后每次推送都会产生额外成本。
