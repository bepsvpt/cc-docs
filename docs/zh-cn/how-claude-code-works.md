...
* **[自动内存](/zh-CN/memory#auto-memory)。** Claude 在您工作时自动保存的学习内容，如项目模式和您的偏好。MEMORY.md 的前 200 行在每个会话开始时加载。
...
您可以通过终端、[桌面应用](/zh-CN/desktop)、[IDE 扩展](/zh-CN/ide-integrations)、[claude.ai/code](https://claude.ai/code)、[远程控制](/zh-CN/remote-control)、[Slack](/zh-CN/slack) 和 [CI/CD 管道](/zh-CN/github-actions)访问 Claude Code。界面决定了您如何看到和与 Claude 交互，但底层的代理循环是相同的。有关完整列表，请参阅[在任何地方使用 Claude Code](/zh-CN/overview#use-claude-code-everywhere)。
