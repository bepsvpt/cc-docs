...
| **限制令牌预算**           | 设置 [`MAX_THINKING_TOKENS`](/zh-CN/env-vars) 环境变量                              | 将思考预算限制为特定数量的令牌（在 Opus 4.6 和 Sonnet 4.6 上被忽略，除非设置为 0）。示例：`export MAX_THINKING_TOKENS=10000` |
...
**使用较旧的模型**，思考使用固定预算，最多 31,999 个令牌来自您的输出预算。您可以使用 [`MAX_THINKING_TOKENS`](/zh-CN/env-vars) 环境变量限制此，或通过 `/config` 或 `Option+T`/`Alt+T` 切换完全禁用思考。

`MAX_THINKING_TOKENS` 在 Opus 4.6 和 Sonnet 4.6 上被忽略，因为自适应推理控制思考深度。一个例外：设置 `MAX_THINKING_TOKENS=0` 仍然在任何模型上完全禁用思考。要禁用自适应思考并恢复到固定思考预算，请设置 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`。请参阅[环境变量](/zh-CN/env-vars)。
