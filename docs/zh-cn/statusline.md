...
这些示例使用 Bash 脚本，在 macOS 和 Linux 上工作。在 Windows 上，请参阅[Windows 配置](#windows-configuration)以获取 PowerShell 和 Git Bash 示例。
...
你的脚本在每条新的助手消息之后、权限模式更改时或 vim 模式切换时运行。更新在 300ms 处进行防抖，这意味着快速更改会批处理在一起，你的脚本在事情稳定后运行一次。如果在你的脚本仍在运行时触发新的更新，则会取消正在进行的执行。如果你编辑你的脚本，更改在你与 Claude Code 的下一次交互触发更新之前不会出现。
...
<Accordion title="完整 JSON 架构">
  ...
  * `worktree`：仅在 `--worktree` 会话期间出现。存在时，`branch` 和 `original_branch` 对于基于钩子的 worktree 也可能不存在
...
为缓存文件使用稳定的固定文件名，如 `/tmp/statusline-git-cache`。每个状态行调用作为新进程运行，所以像 `$$`、`os.getpid()` 或 `process.pid` 这样的基于进程的标识符每次都产生不同的值，缓存永远不会被重用。
...
* **缓存慢速操作**：你的脚本在活跃会话期间频繁运行，所以像 `git status` 这样的命令可能会导致延迟。请参阅[缓存示例](#cache-expensive-operations)以了解如何处理此问题。
...
* 在第一个 API 响应完成之前，字段可能为 `null`
* 在你的脚本中使用回退处理 null 值，如 jq 中的 `// 0`
* 如果多条消息后值仍为空，请重新启动 Claude Code

**上下文百分比显示意外值**

* 使用 `used_percentage` 获得准确的上下文状态，而不是累积总计
* `total_input_tokens` 和 `total_output_tokens` 在整个会话中累积，可能超过上下文窗口大小
