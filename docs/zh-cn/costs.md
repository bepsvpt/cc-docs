...
扩展思考默认启用，预算为 31,999 个令牌，因为它显著改进了复杂规划和推理任务的性能。但是，思考令牌作为输出令牌计费，因此对于不需要深度推理的更简单任务，您可以通过在 `/model` 中为 Opus 4.6 降低[努力级别](/zh-CN/model-config#adjust-effort-level)、在 `/config` 中禁用思考或降低预算（例如，`MAX_THINKING_TOKENS=8000`）来降低成本。
