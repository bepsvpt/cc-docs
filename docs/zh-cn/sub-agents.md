...
<Steps>
  ...
  <Step title="使用 Claude 生成">
    ...
    Claude 生成系统提示和配置。如果您想自定义它，请按 `e` 在编辑器中打开它。
  </Step>
...
当代理作为主线程运行时，使用 `claude --agent`，它可以使用 Agent 工具生成 subagents。要限制它可以生成的 subagent 类型，请在 `tools` 字段中使用 `Agent(agent_type)` 语法。
...
要允许生成任何 subagent 而不受限制，请使用不带括号的 `Agent`：
...
<Warning>
  谨慎使用 `bypassPermissions`。它跳过所有权限检查，允许 subagent 执行任何操作而无需批准。
</Warning>
...
`memory` 字段为 subagent 提供一个在对话中存活的持久目录。Subagent 使用此目录随时间积累知识，例如代码库模式、调试见解和架构决策。
...
您可以通过将 subagents 添加到 [settings](/zh-CN/settings#permission-settings) 中的 `permissions.deny` 数组来防止 Claude 使用特定 subagents。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 与 subagent 的 name 字段匹配。
...
有关权限规则的更多详细信息，请参阅 [Permissions 文档](/zh-CN/permissions#tool-specific-permission-rules)。

### 为 subagents 定义 hooks

Subagents 可以定义在 subagent 生命周期期间运行的 [hooks](/zh-CN/hooks)。有两种方式来配置 hooks：
...
所有 [hook 事件](/zh-CN/hooks#hook-events) 都受支持。subagents 最常见的事件是：
...
* **后台 subagents** 在您继续工作时并发运行。启动前，Claude Code 会提示您输入 subagent 需要的任何工具权限，确保它具有必要的批准。一旦运行，subagent 继承这些权限并自动拒绝任何未预先批准的内容。如果后台 subagent 需要提出澄清问题，该工具调用失败，但 subagent 继续。
...
要禁用所有后台任务功能，请将 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` 环境变量设置为 `1`。请参阅 [Environment variables](/zh-CN/settings#environment-variables)。
...
<Warning>
  当 subagents 完成时，它们的结果返回到主对话。运行许多 subagents，每个都返回详细结果，可能会消耗大量上下文。
</Warning>
...
对于关于对话中已有内容的快速问题，请使用 [`/btw`](/zh-CN/interactive-mode#side-questions-with-btw) 而不是 subagent。它看到您的完整上下文但没有工具访问权限，答案被丢弃而不是添加到历史记录。
...
* **会话持久性**：Subagent 转录在其会话中持久化。您可以通过恢复同一会话来在重启 Claude Code 后 [恢复 subagent](#resume-subagents)。
* **自动清理**：转录根据 `cleanupPeriodDays` 设置进行清理（默认：30 天）。

#### 自动压缩

Subagents 支持使用与主对话相同的逻辑进行自动压缩。默认情况下，自动压缩在大约 95% 容量时触发。要更早触发压缩，请将 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 设置为较低的百分比（例如 `50`）。有关详细信息，请参阅 [environment variables](/zh-CN/settings#environment-variables)。
...
一个只读 subagent，审查代码而不修改它。此示例展示了如何设计一个专注的 subagent，具有有限的工具访问权限（无 Edit 或 Write）和详细的提示，指定确切要查找的内容以及如何格式化输出。
...
一个用于数据分析工作的特定领域 subagent。此示例展示了如何为典型编码任务之外的专门工作流创建 subagents。它明确设置 `model: sonnet` 以进行更强大的分析。
...
一个允许 Bash 访问但验证命令以仅允许只读 SQL 查询的 subagent。此示例展示了如何使用 `PreToolUse` hooks 进行条件验证，当您需要比 `tools` 字段更精细的控制时。
...
Hook 通过 stdin 接收 JSON，Bash 命令在 `tool_input.command` 中。退出代码 2 阻止操作并将错误消息反馈给 Claude。有关退出代码和 [Hook input](/zh-CN/hooks#pretooluse-input) 的完整输入架构的详细信息，请参阅 [Hooks](/zh-CN/hooks#exit-code-output)。
