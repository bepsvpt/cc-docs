...
| Plugin 的 `agents/` 目录 | 启用 plugin 的位置 | 4（最低）    | 与 [plugins](/zh-CN/plugins) 一起安装 |
...
| `model`           | No       | [Model](#choose-a-model) 使用：`sonnet`、`opus`、`haiku`、完整模型 ID（例如，`claude-opus-4-6`）或 `inherit`。默认为 `inherit`                                                                    |
| `permissionMode`  | No       | [Permission mode](#permission-modes)：`default`、`acceptEdits`、`dontAsk`、`bypassPermissions` 或 `plan`                                                                           |
...
| `mcpServers`      | No       | [MCP servers](/zh-CN/mcp) 对此 subagent 可用。每个条目要么是引用已配置服务器的服务器名称（例如，`"slack"`），要么是内联定义，其中服务器名称为键，完整的 [MCP server config](/zh-CN/mcp#configure-mcp-servers) 为值                   |
...
<Warning>
  谨慎使用 `bypassPermissions`。它跳过权限提示，允许 subagent 在没有批准的情况下执行操作。对 `.git`、`.claude`、`.vscode` 和 `.idea` 目录的写入仍然会提示确认，除了 `.claude/commands`、`.claude/agents` 和 `.claude/skills`。有关详细信息，请参阅 [permission modes](/zh-CN/permission-modes#skip-all-checks-with-bypasspermissions-mode)。
</Warning>

如果父级使用 `bypassPermissions`，这优先并且无法被覆盖。如果父级使用 [auto mode](/zh-CN/permission-modes#eliminate-prompts-with-auto-mode)，subagent 继承 auto mode，其 frontmatter 中的任何 `permissionMode` 被忽略：分类器使用与父会话相同的块和允许规则评估 subagent 的工具调用。
...
* Subagent 的系统提示还包括内存目录中 `MEMORY.md` 的前 200 行，以及如果 `MEMORY.md` 超过 200 行则策划 `MEMORY.md` 的说明。
...
由启用的 [plugin](/zh-CN/plugins) 提供的 Subagents 在类型提前中显示为 `<plugin-name>:<agent-name>`。您也可以手动输入提及而不使用选择器：`@agent-<name>` 用于本地 subagents，或 `@agent-<plugin-name>:<agent-name>` 用于 plugin subagents。
...
对于关于对话中已有内容的快速问题，使用 [`/btw`](/zh-CN/interactive-mode#side-questions-with-btw) 而不是 subagent。它看到您的完整上下文但没有工具访问，答案被丢弃而不是添加到历史记录。
...
当 subagent 完成时，Claude 接收其代理 ID。Claude 使用 `SendMessage` 工具，将代理的 ID 作为 `to` 字段来恢复它。要恢复 subagent，要求 Claude 继续之前的工作：
