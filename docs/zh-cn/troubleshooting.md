...
如果您安装了较旧版本的 Claude Desktop，它可能在 `WindowsApps` 目录中注册一个 `Claude.exe`，其 PATH 优先级高于 Claude Code CLI。运行 `claude` 会打开桌面应用而不是 CLI。
...
如果浏览器在登录期间不会自动打开，按 `c` 将 OAuth URL 复制到您的剪贴板，然后手动将其粘贴到您的浏览器中。
...
如果您在 JetBrains 终端中使用 Claude Code 且 `Esc` 键不会按预期中断代理，这可能是由于 JetBrains 默认快捷键的冲突。
...
3. **指定格式偏好**：在您的提示或项目[内存](/zh-CN/memory)文件中包含格式要求。
