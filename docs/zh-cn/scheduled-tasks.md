...
任务是会话范围的：它们存在于当前 Claude Code 进程中，当您退出时就会消失。对于需要在重启后继续运行且无需活跃终端会话的持久调度，请参阅 [Desktop 计划任务](/zh-CN/desktop#schedule-recurring-tasks) 或 [GitHub Actions](/zh-CN/github-actions)。

## 使用 /loop 计划重复提示词

`/loop` [bundled skill](/zh-CN/skills#bundled-skills) 是计划重复提示词的最快方式。传递一个可选的间隔和一个提示词，Claude 会设置一个在后台运行的 cron 作业，同时会话保持打开。

```text  theme={null}
/loop 5m check if the deployment finished and tell me what happened
```

Claude 解析间隔，将其转换为 cron 表达式，计划作业，并确认节奏和作业 ID。
...
支持的单位是 `s` 表示秒、`m` 表示分钟、`h` 表示小时、`d` 表示天。秒数向上舍入到最近的分钟，因为 cron 的粒度为一分钟。不能均匀分割其单位的间隔，例如 `7m` 或 `90m`，会舍入到最近的整洁间隔，Claude 会告诉您它选择了什么。

### 循环另一个命令

计划的提示词本身可以是命令或 skill 调用。这对于重新运行您已经打包的工作流很有用。
...
对于一次性提醒，用自然语言描述您想要的内容，而不是使用 `/loop`。Claude 计划一个单次触发的任务，在运行后删除自己。
...
调度程序每秒检查一次到期的任务，并以低优先级将其入队。计划的提示词在您的回合之间触发，而不是在 Claude 正在响应时。如果 Claude 在任务到期时忙碌，提示词会等到当前回合结束。
...
重复任务在创建后 3 天自动过期。任务最后触发一次，然后删除自己。这限制了被遗忘的循环可以运行多长时间。如果您需要重复任务持续更长时间，请在过期前取消并重新创建它，或使用 [Desktop 计划任务](/zh-CN/desktop#schedule-recurring-tasks) 进行持久调度。
...
在您的环境中设置 `CLAUDE_CODE_DISABLE_CRON=1` 以完全禁用调度程序。cron 工具和 `/loop` 变得不可用，任何已计划的任务都停止触发。有关禁用标志的完整列表，请参阅 [环境变量](/zh-CN/settings#environment-variables)。
...
对于需要无人值守运行的 cron 驱动自动化，使用带有 `schedule` 触发器的 [GitHub Actions 工作流](/zh-CN/github-actions)，或如果您想要图形化设置流程，使用 [Desktop 计划任务](/zh-CN/desktop#schedule-recurring-tasks)。
